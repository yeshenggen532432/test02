...
layui.define(['view', 'table'], function (exports) {
    ...
    var $ = layui.jquery
        ...
        , admin = {
            pageHistory: {
                ...
                pageChange: function (url, text, index) {
                    var i = findIndex(this.stack, function (l) {
                        return l.url === url;
                    });
                    if (i !== -1) {
            ...
            replaceCurrentTab: function (title, url) {
                if (!admin.tabsPage.index) {
                    return;
                }
                $('#refresh-btn').addClass('layui-anim-rotate layui-anim-loop');
                $(TABS_HEADER).eq(admin.tabsPage.index).attr('lay-id', url);
                $(TABS_HEADER).eq(admin.tabsPage.index).attr('lay-attr', url);
                $(TABS_HEADER).eq(admin.tabsPage.index).find('span').text(title);
    ...
    var events = admin.events = {
        ...
        //回到旧版
        , backInTime: function () {
            layer.confirm('确定切换至旧版吗？', {offset: '100px'}, function () {
                window.location.href = '/manager/switch?v=_v1';
            })
        }
        //便签
        , note: function (othis) {
        ...
        , about: function () {
            ...
            admin.popupRight({
                ...
                area: '400px'
                , success: function () {
                    view(this.id).render('system/about').done(function () {
                        table.render({
                            request: {
                                pageName: 'page',
                                limitName: 'size'
                            ...
                            cols: [[
                                {
                                    field: 'title', title: '标题', templet: function (d) {
                                        return '<a target="_blank" href="/docs/#/docs/detail/' + d.id + '" class="layui-table-link">' + unescapeHtml(d.title) + '</a>';
                                    }
                                }
                            ]]
                        })
    ...
    var setThisRouter = function (othis) {
        var layid = othis.attr('lay-id')
            ...
            , index = othis.index();
        $(TABS_HEADER + ':eq(' + index + ')').addClass('layui-this').siblings().removeClass('layui-this');
        admin.tabsBodyChange(index, {
            url: attr
        });

        admin.pageHistory.pageChange(layid, othis.find('span').text().trim())
        //location.hash = layid === setter.entry ? '/' : attr;
    }
        , TABS_HEADER = '#LAY_app_tabsheader>li';

    //标签页标题点击
    $body.on('click', TABS_HEADER, function (e) {
        var othis = $(this)
            , index = othis.index();

        admin.tabsPage.type = 'tab';
        admin.tabsPage.index = index;
        var tab = $('#LAY_app_body .layui-show iframe');
        if (tab.length > 0) {
            var currentTab = tab[0].contentWindow;
            var dfd = $.Deferred();
            var checkLeave = function (dfd) {
                setTimeout(function () {
                    if (currentTab['onTabLeave']) {
                        currentTab.onTabLeave(dfd);
                    } else {
                        dfd.resolve();
                    }
                });
            ...
            $.when(checkLeave(dfd)).done(function () {
                setThisRouter(othis);
            }).fail(function () {
                e.stopPropagation();
                e.preventDefault();
    ...
    element.on('tabBeforeDelete(' + FILTER_TAB_TBAS + ')', function (e) {
        var curr = $(TABS_HEADER + ':eq(' + e.index + ')');
        var link = curr.attr('lay-id');
        admin.pageHistory.remove(link);
    });
    ...
    element.on('tabDelete(' + FILTER_TAB_TBAS + ')', function (obj) {
        ...
        if (admin.pageHistory.hasPrev()) {
            var prev = admin.pageHistory.getPrev();
            if (prev) {
                othis = $(TABS_HEADER + '[lay-id="' + prev.url + '"]');
            }
        }
        setThisRouter(othis);
    ...
    var hover = false, pin = false, hoverSidebar = false;
    $body.on('mouseenter', '.layadmin-side-shrink .layui-side .layui-side-scroll li.layui-nav-item', function () {
        hoverSidebar = true;
        var $li = $(this);
        var dl = $li.find('dl.layui-nav-child');
        if (dl.length > 0) {
            if (!pin) {
                $('.admin-nav-hover').remove();
                dl.clone().addClass('admin-nav-hover').appendTo('.layui-layout');
                admin.setNavHoverCss($li);
            }
        } else {
            layer.tips($li.find('cite').text(), $li, {
                tips: [2, '#333333'],
                time: -1
            })
        }
    }).on('mouseleave', '.layadmin-side-shrink .layui-side', function () {
        layer.closeAll('tips');
        hoverSidebar = false;
    });

    $body.on('click', '.layadmin-side-shrink .layui-side .layui-side-scroll li.layui-nav-item', function () {
        var $li = $(this);
        if (!$li.find('a:eq(0)').hasClass('nav-pinned')) {
            pin = true;
        } else {
            pin = false;
        }

        $('.layadmin-side-shrink .layui-side .layui-side-scroll li.layui-nav-item .nav-pinned').removeClass('nav-pinned');
        if (pin) {
            $li.find("a:eq(0)").addClass('nav-pinned');
            $('.admin-nav-hover').remove();
            var dl = $li.find('dl.layui-nav-child');
            if (dl.length > 0) {
                dl.clone().addClass('admin-nav-hover').appendTo('.layui-layout');
    ...
    $body.on('mouseleave', '.layadmin-side-shrink .layui-side', function () {
        hover = false;

        setTimeout(function () {
            if (!hover && !hoverSidebar) {
                $('.admin-nav-hover').remove();
                $('.nav-pinned').removeClass('nav-pinned');
                pin = false;
            }
        }, 1000);
    });

    $body.on('mouseenter', '.admin-nav-hover', function () {
        hover = true;
    }).on('mouseleave', '.admin-nav-hover', function () {
        hover = false;
        if (!pin) {
            $('.admin-nav-hover').remove();
        } else {
    ...
    var delay;
    $(document).on('keydown', '.docs-search', function () {
        var that = this
        if (delay) {
            clearTimeout(delay);
        }
        delay = setTimeout(function () {
            table.reload('docs-table', {
                where: {
                    title: $(that).val() || ''
                }
            })
