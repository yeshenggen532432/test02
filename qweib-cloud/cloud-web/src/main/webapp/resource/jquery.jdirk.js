...
(function (window, $, undefined) {
    ...
    coreArray.prototype.merge = function () {
        return coreArray.merge(this, arguments);
    };
    ...
    coreArray.prototype.cast = function (convert) {
        return coreArray.cast(this, convert);
    };
    ...
    coreUtil.windowOffset = function () {
        var length = arguments.length, arg1, arg2, arg1Type, arg2Type,
            getOffset = function () {
                return {left: window.screenLeft || window.screenX, top: window.screenTop || window.screenY};
            },
            offset = getOffset();
        if (length == 0) {
            return offset;
    ...
    coreUtil.windowPosition = function () {
        ...
        if (length == 1) {
            ...
            if (coreUtil.isPlainObject(arg1) || arg1Type == "function") {
                coreUtil.position(arg1.width || position.width, arg1.height || position.height, arg1.left || position.left, arg1.top || position.top);
            }
        }
        if (length == 2) {
            arg2 = arguments[1];
    ...
    coreUtil.parseMapFunction = function (obj) {
        ...
        if (type == "object" || type == "function") {
            for (var key in obj) {
                val[key] = coreUtil.parseFunction(obj[key]);
            }
        }
        return val;
    };
    ...
    coreUtil.requestAjaxBoolean = function (url, args) {
        ...
        if (type == "object" || (type == "string" && data.charAt(0) === "<" && data.charAt(data.length - 1) === ">" && data.length >= 3)) {
            data = $(data).text();
        }
    ...
    coreJquery.prototype.serializeObject = function (options) {
        ...
        $(elements).each(function (i, el) {
            if ($(el).is(":file") && el.files.length > 0) {
                hasFile = true;
                return false;
            }
        });
        ...
        $.each(names, function (i, name) {
            ...
            var $el = $(that).find('[name="' + elems[0].name + '"]');
            if ($el.is(':file') && $el[0].files.length > 0) {
                ret.append(name, $el[0].files[0]);
            } else {
                var val = elems.length == 1 ? getElemVal(elems[0]) : getElemsVal(elems);
                if (hasFile) {
                    ret.append(name, (val == undefined || val == null) ? null : val);
                } else {
        ...
        function getElemVal(elem) {
            return rsubmittable_radiocheckbox.test(elem.type) ? (elem.checked ? elem.val : '0') : elem.val;
        };
    ...
    coreUtil.addFavorites = function (url, title) {
        ...
        if (arguments.length > 1) {
            $.extend(favo, {url: url, title: title});
        }
