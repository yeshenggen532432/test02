...
{
	var path = "queryIoTypeList?typeName=预付账款";
	//var token = $("#tmptoken").val();
	//alert(token);
	var topType = 0;
	$.ajax({
...
{
	var path = "queryIoItemList?typeName=预付账款";
	//var token = $("#tmptoken").val();
	//alert(token);
...
function draftSave(){
	...
	var path = "addFinPreOut";
	var token = $("#tmptoken").val();
	if(!confirm('是否确定暂存？'))return;
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"proId":proId,"status":0,"accId":accId,"proType":proType,"proName":proName,"remarks":remarks1,"billTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function saveAudit(){
	...
	$.ajax({
		url: path,
		type: "POST",
		data : {"token":token,"id":billId,"proId":proId,"status":1,"accId":accId,"proType":proType,"proName":proName,"remarks":remarks1,"billTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
		dataType: 'json',
		async : false,
		success: function (json) {
			if(json.state){
