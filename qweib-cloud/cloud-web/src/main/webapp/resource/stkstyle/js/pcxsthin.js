...
//    					}
        				if($("#stkNamespan").text()==json.list[i].stkName){
        					$("#stkId").val(json.list[i].id);
        				}
        			}
...
function providerClick(trobj)
{
	...
	stId = $(this).find("input[name='cstId']").val();
...
function customerclick(trobj)
{
	...
	//$("#csttel").val(tel);
	//$("#cstaddress").val(address);
	var saleId = $(trobj.cells[3]).find("#saleId").text();
	var saleNm = $(trobj.cells[3]).find("#saleNm").text();
	$("#empNm").html(saleNm);
	$("#empId").val(saleId);
	$(".pcl_chose_people").hide();
	//queryOrder();
...
function auditClick()
{
	...
	if(status == "已审")
		{
		   $.messager.alert('消息',"该单据已经审核");
		  return;
		}
	if(status == "作废")
		{
		 $.messager.alert('消息',"该单据已经作废");
		return;
		}
	auditProc();
}
...
function deleteChoose(lineObj)
{
	...
	if(confirm('确定删除？')){
		// $.messager.alert('消息',lineObj.innerHTML);
	...
	$.ajax({
        ...
        success: function (json) {
        	if(json.state){
                ...
                for(var i=0;i<size;i++){
                    var data = {
                        "wareId": json.rows[i].wareId,
                        "wareNm": json.rows[i].wareNm,
                        "wareGg": json.rows[i].wareGg,
                        "wareCode": json.rows[i].wareCode,
                        "hsNum": json.rows[i].hsNum,
                        "qty":json.rows[i].wareNum,
                        "unitName":json.rows[i].wareDw,
                        "price": json.rows[i].wareDj,
                        "sunitFront": json.rows[i].sunitFront,
                        "sunitPrice": json.rows[i].sunitPrice,
                        "maxNm": json.rows[i].wareDw,
                        "minUnitCode": json.rows[i].minUnitCode,
                        "maxUnitCode": json.rows[i].maxUnitCode,
                        "minNm": json.rows[i].minUnit,
                        "hsNum": json.rows[i].hsNum,
                        "wareDw2":json.rows[i].wareDw2,
                        "beUnit":json.rows[i].beUnit
                    }
                    var row = addTabRow(1);
                    setRowData(row,data);
                    if(json.rows[i].beUnit!=""){
                        $(row).find("select[name='beUnit']").val(json.rows[i].beUnit);
                    }else{
                            var wareDw2 =json.rows[i].wareDw2;
                            if(json.rows[i].wareDw==wareDw2){
                                $(row).find("select[name='beUnit'] option:first").prop("selected", 'selected');
                            }
                            if(json.rows[i].minUnit==wareDw2){
                                $(row).find("select[name='beUnit'] option:eq(1)").prop("selected", 'selected');
                            }
                    }
...
function submitStk(){
	...
	// $.messager.alert('消息',outTime);
	
	var discount = $("#discount").val();
	var orderId = $("#orderId").val();
	var orderNo = $("#orderNo").val();
	var empId = $("#empId").val();
	var empNm = $("#empNm").html();
	...
	if(wareList.length == 0)
		{
			 $.messager.alert('消息',"请选择商品");
			return;
		}
	if(stkId == 0)
		{
			 $.messager.alert('消息',"请选择仓库");
			return;
		}
	
	if(proId == 0)
		{
			 $.messager.alert('消息',"请选择退货对象");
			return;
		}
	var path = "addStkIn";
	var token = $("#tmptoken").val();
	if(!confirm('保存后将不能修改，是否确定保存？'))return;
	// $.messager.alert('消息',JSON.stringify(wareList));
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"proId":proId,"proType":proType,"proName":proName,"driverId":driverId,"vehId":vehId,"stkId":stkId,"empId":empId,"empNm":empNm,"remarks":remarks,"inDate":inTime,"inType":"销售退货","orderId":orderId,"orderNo":orderNo,"discount":discount,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function draftSave(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"proId":proId,"proType":proType,"driverId":driverId,"vehId":vehId,"proName":proName,"stkId":stkId,"empId":empId,"empNm":empNm,"remarks":remarks,"inDate":inTime,"inType":"销售退货","orderId":orderId,"orderNo":orderNo,"discount":discount,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function auditProc()
{
	...
	if(billId == 0)
		{
			$.messager.alert('消息',"没有可审核的单据");
			return;
		}
	parent.closeWin('退货入库收货' + billId);
	parent.add('退货入库收货' + billId,'manager/showstkincheck?dataTp=1&billId=' + billId);
...
function reAudit()
{
	...
	$.ajax({
		...
		success: function (json) {
			if(json.state){
				alert("反审批成功");
				window.location.href="showstkin?billId=" + billId;
			}
			else
			{
...
function printClick()
{
	...
	if(billStatus == 2)
		{
		   $.messager.alert('消息',"单据已经作废");
		  return;
		}
	var billId = $("#billId").val();
	if(billId == 0)
		{
			 $.messager.alert('消息',"没有可打印的单据");
			return ;
		}
	if(isModify){
		$.messager.alert('消息','单据已修改，请先保存!','info');
...
function cancelClick()
{
    ...
    $.ajax({
        ...
        success: function (json) {
        	...
        	{
        		 $.messager.alert('消息',"作废失败" + json.msg);
        	}
        }
    });
}
