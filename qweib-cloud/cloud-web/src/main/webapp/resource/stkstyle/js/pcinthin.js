...
function auditClick()
{
	...
	if(status == "已审")
		{
			$.messager.alert('消息',"该单据已经审核！",'info');
		  return;
		}
	if(status == "作废")
		{
...
function submitStk(){
	...
	if(proId == 0)
		{
			$.messager.alert('消息',"请选择退货对象！",'info');
			return;
		}
	var path = "addStkIn";
	var token = $("#tmptoken").val();
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"proId":proId,"proType":proType,"proName":proName,"stkId":stkId,"remarks":remarks,"inDate":inTime,"inType":"采购退货","discount":discount,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function auditProc()
{
	...
	parent.closeWin('采购退货收货' + billId);
	parent.add('采购退货收货' + billId,'manager/showstkincheck?dataTp=1&billId=' + billId);
