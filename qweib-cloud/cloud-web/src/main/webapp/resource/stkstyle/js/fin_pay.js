...
$(function(){
	...
	if(costBillId > 0)
		{
		//$('input[name=edtAmt]').attr("readonly","readonly");
		}
	/*
	 * 表单收起按钮
	 * */
...
function newClick()
{
	location = 'toFinPayEdit?costBillId=0';
	if(editstatus != 0)
	{
	  return;
	}
...
function waredbclick(trobj)
{
	...
	$("#more_list tbody").append(
			...
			'<td><input name="edtAmt" type="text" class="pcl_i2" size="50" value="" onchange="countAmt()"/></td>'+
...
function countAmt(orgAmt,obj){
	
	if(orgAmt!=undefined){
		if(orgAmt<obj.value){
			alert("您输入的金额不能大于"+orgAmt);
			obj.value=orgAmt;
		}
	}
	var total = 0;
var trList = $("#chooselist").children("tr");
	...
	$("#payAmt").val(numeral(total).format("0,0.00"));
	isModify = true;
...
function saveAudit(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"depId":depId,"empId":proId,"accId":accId,"status":1,"proType":proType,"proName":proName,"costTerm":costTerm,"costBillId":costBillId,"remarks":remarks,"payTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function draftSave(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"depId":depId,"empId":proId,"accId":accId,"status":0,"proType":proType,"proName":proName,"costTerm":costTerm,"costBillId":costBillId,"remarks":remarks,"payTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
{
	var costId = $(trobj.cells[0]).find("input[name='costId']").val();
	var proId = $(trobj.cells[0]).find("input[name='proId']").val();
	var proType = $(trobj.cells[0]).find("input[name='proType']").val();
	var orderNo = $(trobj.cells[0]).find("span").text();
	var proName = $(trobj.cells[1]).find("span").text();
	var costTerm = $(trobj.cells[1]).find("input").val();
	var depId = $(trobj.cells[2]).find("input").val();
	//var gyn = $(this).find('#pc_lib_name').text(),n = $(this).find('#pc_lib_mes').text();
	//var address = $(this).find('#pc_address').text();
	//var cstId = $(this).find("input[name='cstId']").val();
	
	$("#costBillId").val(costId);
	$("#pc_lib").html(proName);
	$("#depId").val(depId);
	$("#pc_order").html(orderNo);
	$("#costTerm").val(costTerm);
	$("#proId").val(proId);
	$("#proType").val(proType);
	$(".pcl_chose_people").hide();
	queryCostDetail();
...
function queryOrder(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"sdate":sdate,"edate":edate,"proName":khNm,"billStatus":"未付"},
        dataType: 'json',
        async : false,
        success: function (json) {
        		...
        		for(var i = 0;i < size; i++)
        			{
        				text += "<tr onclick=\"orderclick(this)\">" ;
        					...
        					text += "<td><span id=\"pc_lib_order\">" + json.rows[i].billNo + "</span><input  type=\"hidden\" name=\"costId\" value=\"" + json.rows[i].id + "\"/>"
        					+"<input  type=\"hidden\" name=\"proId\" value=\"" + json.rows[i].empId + "\"/>"  +"<input  type=\"hidden\" name=\"proType\" value=\"" + json.rows[i].proType + "\"/></td>	";  
        				
						text += "<td><span id=\"pc_lib_khNm\">" + json.rows[i].proName + "</span><input  type=\"hidden\" name=\"costTerm\" value=\"" + json.rows[i].costTerm + "\"/></td>";
						text += "<td><span id=\"pc_amt\">" + json.rows[i].totalAmt+ "</span><input  type=\"hidden\" name=\"depId\" value=\"" + json.rows[i].depId + "\"/></td>";
	...
	function queryCostDetail()
	{
		var path = "queryFinCostSubToPay";
		var billId = $("#costBillId").val();
	        	...
	        	if(json.state){
	        		...
	        		for(var i = 0;i < size; i++)
	        			...
	        			$("#more_list tbody").append(
	        					'<tr>'+
	        						'<td style="padding-left: 20px;text-align: left;"><img src="'+basePath +'/resource/stkstyle/img/icon19.jpg"class="pcl_ic"/>'+
	        						'<input type="hidden" name="costId" value = "' + json.rows[i].costId + '"/>' + json.rows[i].typeName + '</td>'+
	        						
	        						'<td>' + json.rows[i].itemName + '</td>'+
	        						
	        						'<td><input name="edtAmt" type="text" class="pcl_i2" value="' + json.rows[i].amt + '" onchange="countAmt('+json.rows[i].amt+',this)"/></td>'+
	        						'<td><input name="edtRemarks" type="text" class="pcl_i2" value="' + json.rows[i].remarks + '"/></td>'+
