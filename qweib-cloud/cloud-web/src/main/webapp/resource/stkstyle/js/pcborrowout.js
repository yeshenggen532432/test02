...
function displayXsTpSel()
{
	...
	for (var i=0;i<trList.length;i++) {
	    ...
	    var xsTp = tdArr.eq(2).find("input").val();
	    //alert(xsTp);
	     tdArr.eq(2).find(".pcl_sel2").val(xsTp);//.val();//.options;//.val();
	    //alert(xsTpSel.innerHTML);
	    /*for (i=0; i<all_options.length; i++){
	        if (all_options[i].value == xsTp)
	        	{
	        		all_options[i].selected = true;
	        	}
	    }*/
	    //alert(xsTp);
	    //alert(tdArr.eq(2).find("select").val());
...
function submitStk(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":cstId,"shr":shr,"khNm":khNm,"staff":staff,"empId":empId,"staffTel":staffTel,"proType":proType,"stkId":stkId,"tel":tel,"address":address,"discount":discount,"remarks":remarks1,"orderId":orderId,"outDate":outTime,"outType":"借出出库","pszd":pszd,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function draftSaveStk(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":cstId,"status":status,"shr":shr,"khNm":khNm,"staff":staff,"empId":empId,"staffTel":staffTel,"proType":proType,"stkId":stkId,"tel":tel,"address":address,"discount":discount,"remarks":remarks1,"orderId":orderId,"outDate":outTime,"outType":"借出出库","pszd":pszd,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function auditProc()
{
	...
	parent.close('借出发货单');
	parent.add('借出发货单','manager/showstkoutcheck?dataTp=1&billId=' + billId);
