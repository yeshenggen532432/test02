...
{
	var path = "queryIncomeTypeList?typeName=营业外收入";
	//var token = $("#tmptoken").val();
	//alert(token);
	var topType = 0;
	$.ajax({
...
{
	var path = "queryIncomeItemList?typeName=营业外收入";
	//var token = $("#tmptoken").val();
	//alert(token);
	$.ajax({
        url: path,
...
function draftSave(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":proId,"status":0,"accId":accId,"proType":proType,"khNm":proName,"remarks":remarks1,"recTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function saveAudit(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":proId,"accId":accId,"status":1,"proType":proType,"khNm":proName,"remarks":remarks1,"recTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
