...
function querycustomerPage(filter){
	var filter = $("#searchtext").val();
	var path = "stkchoosecustomer";
	var token = $("#tmptoken").val();
	//alert(token);
...
function saveAudit(){
	...
	var path = "addFinCost";
	var token = $("#tmptoken").val();
	if(!confirm('是否保存并审批？'))return;
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"depId":depId,"status":1,"empId":proId,"proType":proType,"proName":proName,"remarks":remarks1,"costTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function draftSave(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"depId":depId,"empId":proId,"status":0,"proType":proType,"proName":proName,"remarks":remarks1,"costTimeStr":inTime,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
