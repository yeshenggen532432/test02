...
function submitStk(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":cstId,"shr":shr,"khNm":khNm,"staff":staff,"empId":empId,"staffTel":staffTel,"proType":proType,"stkId":stkId,"tel":tel,"address":address,"discount":discount,"remarks":remarks1,"orderId":orderId,"outDate":outTime,"outType":"其它出库","pszd":pszd,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function draftSaveStk(){
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"cstId":cstId,"status":status,"shr":shr,"khNm":khNm,"staff":staff,"empId":empId,"staffTel":staffTel,"proType":proType,"stkId":stkId,"tel":tel,"address":address,"discount":discount,"remarks":remarks1,"orderId":orderId,"outDate":outTime,"outType":"其它出库","pszd":pszd,"wareStr":JSON.stringify(wareList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
...
function auditProc()
{
	...
	parent.close('其它发货单');
	parent.add('其它发货单','manager/showstkoutcheck?dataTp=1&billId=' + billId);
