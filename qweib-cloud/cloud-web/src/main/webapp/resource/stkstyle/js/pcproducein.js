...
function draftSave(){
	...
	var trRelaList = $("#chooselistRela").children("tr");
	var wareCollarList = new Array();
	for (var i=0;i<trRelaList.length;i++) {
		 var qty = trRelaList.eq(i).find("input[name='edtqty']").val();
		 var wareId = trRelaList.eq(i).find("input[name='wareId']").val();
		 var beUnit = trRelaList.eq(i).find("input[name='beUnit']").val();
		 var sourceMainNo = trRelaList.eq(i).find("input[name='sourceMainNo']").val();
		 var sourceMainId = trRelaList.eq(i).find("input[name='sourceMainId']").val();
		 var sourceItemId = trRelaList.eq(i).find("input[name='sourceItemId']").val();
		 var subObj = {
					wareId: wareId,				
					qty: qty,
					beUnit: beUnit,
					sourceItemId:sourceItemId,
					sourceMainId:sourceMainId,
					sourceMainNo:sourceMainNo
			};
		 wareCollarList.push(subObj);
	}
	...
	$.ajax({
        url: path,
        type: "POST",
        data : {"token":token,"id":billId,"proId":proId,"proType":proType,"proName":proName,"stkId":stkId,"remarks":remarks,"inDate":inTime,"inType":"生产入库","discount":discount,"wareStr":JSON.stringify(wareList),"wareCollar":JSON.stringify(wareCollarList)},
        dataType: 'json',
        async : false,
        success: function (json) {
        	if(json.state){
