		...
		<input  type="hidden" name="token" id="tmptoken" value="${token}"/>
			<table>
				<tr>
					...
					<td width="25%">
						<div class="dtbox">
							<a href="javascript:;" id = "choosestatus">未付款</a>
							<img src="<%=basePath %>/resource/stkstyle/img/icon22.png"/>
						</div>
					</td>
					<td width="25%">
			...
			<div class="bt">
				<table>
					<tr>
						...
						<td><a href="javascript:switchToPay();">付款</a></td>
						<td><a href="javascript:switchToFreePay();">核销</a></td>
						<td><a href="javascript:showPayList();">付款明细</a></td>	
	...
	<script type="text/javascript">
		...
		function queryData(pageIndex){
			...
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"proName":searchKey,"sdate":startDate,"edate":endDate,"isPay":isPay,"page":pageIndex,"rows":20},
		        dataType: 'json',
		        async : false,
		        		...
		        		for(var i = 0;i < size; i++)
		        			{
								...
								text += "<td>" + json.rows[i].needPay + "</td>";	
								
								text += "<td>" + json.rows[i].billStatus + "</td>";
								text += "<td>" + json.rows[i].payStatus + "</td>";
		        				text = text + "</tr>";
		...
		function switchToFreePay()
		{
		    ...
		    window.location.href='stkFreePay?token=' + token + '&billId=' + keys;
		}
		function showPayList()
		{
		    ...
		    window.location.href='queryPayPageByBillId?token=' + token + '&billId=' + keys;
		}
