	...
	<body onload="initGrid()">
		...
		<div>
  	    ...
  	    <div id="upDiv" class="easyui-window" style="width:500px;height:100px;padding:10px;"
			...
			<form action="manager/finInitQtJrMain/toUpWljrInitAmt?bizType=${bizType}" id="upFrm" method="post" enctype="multipart/form-data">
		...
		<script type="text/javascript">
		    function initGrid()
		    {
		    	    ...
		    	    cols.push(col);
		    	    $('#datagrid').datagrid({
		    	    	url:"manager/finInitQtJrMain/page",
		    	    	queryParams:{
		    	    		jz:"1",
							billNo:$("#billNo").val(),
							status:$("#status").val(),
		    ...
		    function cancelBill(id){
				  ...
				  if (r) {
					$.ajax({
						url:"manager/finInitQtJrMain/cancelBill",
						type:"post",
						data:"billId="+id,
		    ...
		    function toWlJkModel() {
                if(confirm("是否下载模版?")){
					window.location.href="manager/finInitQtJrMain/toWlJrInitAmtTemplate?bizType=${bizType}";
				}
            }
		    ...
		    function auditBill(id){
				  ...
				  if (r) {
					$.ajax({
						url:"manager/finInitQtJrMain/auditBatch",
						type:"post",
						data:"billId="+id,
						success:function(json){
							if(json.state){
			...
			function toAdd(){
				...
				parent.parent.add(title,'manager/finInitQtJrMain/add?bizType=${bizType}&proType='+proType);
			}
