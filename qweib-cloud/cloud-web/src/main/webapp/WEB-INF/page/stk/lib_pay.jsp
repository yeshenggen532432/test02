...
<html>
	...
	<header class="fixed_top">
		...
		<div class="order_count">
			<ul>
				...
				<li>
					已付款 &nbsp;&nbsp;<a style="color:red;width:60px">${payAmt}</a>&nbsp;&nbsp; 还需付&nbsp;&nbsp;<a style="color:red;width:60px">${needPay}</a>
					
				</li>
				<li>
					 现金
					<div class="order_mes fr">
						<span><input type="text" placeholder="请点击输入" id="cashpay" style="text-align:right"/></span>
					</div>
				</li>
				<li>
					银行存款
					<div class="order_mes fr">
						<span><input type="text" placeholder="请点击输入" id="bankpay" style="text-align:right"/></span>
					</div>
				</li>
				...
				<li>
					微信
					<div class="order_mes fr">
						<span><input type="text" placeholder="请点击输入" id="wxpay" style="text-align:right"/></span>
					</div>
				</li>
				<li>
					支付宝
					<div class="order_mes fr">
						<span><input type="text" placeholder="请点击输入" id="zfbpay" style="text-align:right"/></span>
					</div>
				</li>
	...
	function submitPay()
	{
		...
		if(parseFloat(needPay)< parseFloat(cash)+ parseFloat(bank) + parseFloat(wx) + parseFloat(zfb))
		{
			alert("支付金额不能大于应付金额");
			return;
		}
		if(parseFloat(cash)+ parseFloat(bank) + parseFloat(wx) + parseFloat(zfb) == 0)
			{
				alert("请输入要付款的金额");
				return;
			}
		if (!confirm('是否确认付款？'))return;
		var path = "payProc";
		...
		$.ajax({
	        url: path,
	        type: "POST",
	        data : {"token":token,"proId":proId,"billId":billId,"remarks":remarks,"cash":cash,"bank":bank,"zfb":zfb,"wx":wx},
	        dataType: 'json',
	        async : false,
