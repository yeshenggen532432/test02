...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		<table id="datagrid" class="easyui-datagrid" fit="true" singleSelect="false"
			url="manager/finUnitPage" title="" iconCls="icon-save" border="false"
		...
		<script type="text/javascript"><!--
			...
			function queryprovider(){
				...
				$("#datagrid").datagrid('load',{
					url:"manager/finUnitPage?proName="+$("#proName").val()
			...
			function toaddprovider(){
				location.href="${base}/manager/tofinunitedit";
			}
			//修改
			function getSelected(){
			    var row = $('#datagrid').datagrid('getSelected');
				if (row){
					  location.href="${base}/manager/tofinunitedit?Id="+row.id;
				 }else{
					alert("请选择要修改的行！");
				}
			}
		    ...
		    function toDel() {
				...
				if (ids.length > 0) {
					$.messager.confirm('确认', '您确认想要删除记录吗？', function(r) {
						if (r) {
							$.ajax( {
								url : "manager/deletefinunit",
								data : "ids=" + ids,
								type : "post",
								success : function(json) {
									if(json==2){
										showMsg("删除失败，该往来单位已使用！");
									}
									if (json == 1) {
										showMsg("删除成功");
										$("#datagrid").datagrid("reload");
		   ...
		   function updateIsTy(_this,id,isTy){
			   alert(id);
				$.ajax({
					url:"manager/updateFinUnitIsUse",
					type:"post",
