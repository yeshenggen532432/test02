...
<html style="font-size: 50px;">
			...
			<div class="pcl_lib_out">
				<div class="pcl_menu_box">
					<div class="menu_btn">
						...
						<div class="item" id="btnauditrk" style="display:${(stkZzcx.status eq 0)?'':'none' }">
							<a href="javascript:auditRk();">
								<img src="<%=basePath %>/resource/stkstyle/img/nicon2.png"/>
								<img src="<%=basePath %>/resource/stkstyle/img/nicon2a.png"/>
								<p>原料入库</p>
							</a>
						...
						<div class="pcl_right_edi" style="width: 500px;">
							<div class="pcl_right_edi_w">
								...
								<input type="text" id="billNo" name="billNo"   style="color:green;width:160px;font-size: 14px" readonly="readonly" value="${stkZzcx.billNo}"/>
								  <c:choose>
								  	<c:when test="${stkZzcx.status eq -2}"><input type="text" id="billstatus"  value="暂存"   style="color:red;width:80px" readonly="readonly"/></c:when>
								  	<c:when test="${stkZzcx.status eq 0}"><input type="text" id="billstatus"  value="原料待入库"   style="color:red;width:80px" readonly="readonly"/></c:when>
								  	<c:when test="${stkZzcx.status eq 1}"><input type="text" id="billstatus"  value="已入库"   style="color:red;width:80px" readonly="readonly"/></c:when>
								  	<c:when test="${stkZzcx.status eq 2}"><input type="text" id="billstatus"  value="已作废"   style="color:red;width:80px" readonly="readonly"/></c:when>
									  <c:otherwise>
					...
					<table>
						<tbody>
							...
							<tr>
								...
								<td colspan="5">
									<div class="pcl_txr">
										<textarea name="remarks" id="remarks" value="${stkZzcx.remarks}"></textarea>
									</div>
								</td>
							</tr>
						</tbody>
			    ...
			    <div title="拆卸商品" style="padding:10px;">
						...
						<table id="more_list">
							<thead>
								<tr>
									...
									<td>单位</td>
									<td>数量</td>
									<td style="display:${stkZzcx.status eq -2?'none':''}">单价</td>
									<td style="display:${stkZzcx.status eq -2?'none':''}">金额</td>
									<td>操作</td>
								</tr>
							</thead>
							<tbody id="chooselist">
								...
								<td style="display:${stkZzcx.status eq -2?'none':''}">
								<input id="price${s.index }" name="subList[${s.index }].price" type="text" readonly="readonly" class="pcl_i2" value="${item.price}" onchange="countAmt()"/>
								</td>
								<td style="display:${stkZzcx.status eq -2?'none':''}">
								...
								<td>
								<c:if test="${stkZzcx.status eq -2 }">
								<a href="javascript:;"onclick="deleteChoose(this);" class="pcl_del">删除</a>
								</c:if>
								</td>
								</tr>
								<script>
				...
				<%--<a class="easyui-linkbutton" iconCls="icon-add" id="mateBtnId" plain="true" style="display:${(empty stkZzcx.id or stkZzcx.id eq 0)?'':'none' }" href="javascript:dialogSelectWare(1);">添加商品</a>--%>
					 <a class="easyui-linkbutton" iconCls="icon-add" id="mateBtnId" plain="true"  href="javascript:dialogSelectWare(1);">添加商品</a>
				</div>
				<div class="pcl_ttbox1 clearfix">
					<div class="pcl_lfbox1">
						<table id="more_list1">
							...
							<tbody id="chooselist">
								...
								<td>
								<input style="display:${stkZzcx.status eq -2?'none':''}" id="amt${s.index }" name="itemList[${s.index }].amt" readonly="readonly" type="text" class="pcl_i2" value="${item.amt}" />
								</td>
								<td>
								...
								<td><a href="javascript:;"onclick="deleteChoose(this);" class="delMateClass" style="display:${(empty stkZzcx.id)?'':'none' }">删除</a></td>
								</tr>
 								</c:forEach>
							</tbody>
						</table>
	...
	function setTabRowData(json){
		...
		$("#more_list tbody").append(
				'<tr>'+
					...
					'<td style="display:none"><input id="price'+rowIndex+'" readonly="readonly" name="subList['+down+'].price" type="text" class="pcl_i2" value="' + price + '" onchange="countAmt()"/></td>'+
	...
	function setRelaTabRowData(json){
		...
		$("#more_list1 tbody").append(
				'<tr>'+
					...
					op
					+ '</td>'+
					'<td><a href="javascript:;"onclick="deleteChooseRela(this);" class="delMateClass">删除</a></td>'+
				'</tr>'
			);
		rowIndex++;
		countRelaAmt();
	...
	function audit(){
		...
		$.messager.confirm('确认', '是否确定拆卸产品出库?确定完成后在点击【原料入库】按钮,关联原料将会进入库存',function(r){
			if(r){
				$.ajax({
			        ...
			        success: function (json) {
			        	if(json.state){
			        		...
			        		$("#status").val(0);
			        		$("#billstatus").val("拆卸产品出库成功");
			        		/*
			        		$("#btnSave").hide();
			        		$("#btnAuditSh").show();*/
			        		alert("拆卸产品出库成功");
			        		window.location.href="<%=basePath %>/manager/stkZzcx/show?billId="+ billId;
			        	}else{
			        		alert(json.msg);
			        	}
			        }
	...
	function auditRk(){
		...
		$.messager.confirm('确认', '是否确定入库?',function(r){
			if(r){
				document.savefrm.action="<%=basePath %>/manager/stkZzcx/auditRk";
				$("#savefrm").form('submit',{
					success:function(data){
						var json = eval("("+data+")");
						if(json.state){
