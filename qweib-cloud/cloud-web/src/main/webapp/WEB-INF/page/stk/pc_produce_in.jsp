...
<html style="font-size: 50px;">
	...
	<head>
		...
		<script src="<%=basePath %>/resource/stkstyle/js/pcproducein.js" type="text/javascript" charset="utf-8"></script>
...
var basePath = '<%=basePath %>';
	...
	<body>
		...
		<div class="center">
			<div class="pcl_lib_out">
			    ...
			    <div title="生产商品" style="padding:10px;">
				
				<div id="tb" style="padding:5px;height:auto">
				<a class="easyui-linkbutton" iconCls="icon-add" plain="true" href="javascript:dialogSelectWare();">入库商品</a>
				</div>
						...
						<table id="more_list">
							<thead>
								<tr>
									...
									<td>产品规格</td>
									<td>单位</td>
									<td style="display: ${permission:checkUserFieldDisplay('stk.stkIn.lookqty')}">入库数量</td>
							...
							<tbody id="chooselist">
								...
								<td style="display: ${permission:checkUserFieldDisplay('stk.stkIn.lookqty')}"><input name="edtqty" type="text" class="pcl_i2" value="${item.qty}" onchange="countAmt()"/></td>
								<td style="display:none"><input name="edtprice" type="text" class="pcl_i2" value="${item.price}" onchange="countAmt()"/></td>
								<td style="display:none"><input name="amt" type="text" class="pcl_i2" value="${item.amt}" onchange="countPrice()"/></td>
				...
				<a class="easyui-linkbutton" iconCls="icon-add" plain="true" href="javascript:dialogCollarWare();">关联原料</a>
				</div>
				
				<div class="pcl_ttbox1 clearfix">
					<div class="pcl_lfbox1">
						<table id="more_list_rela">
							<thead>
								<tr>
									<td>产品名称</td>
									<td>产品规格</td>
							...
							<tbody id="chooselistRela">
								<c:forEach items="${items}" var="item" >
  								<tr>
								<td style="padding-left: 20px;text-align: left;display: none">
								<input type="hidden" name="wareId" value = "${item.wareId}"/>
								</td>
								<td><input name="wareNm" type="text" class="pcl_i2" style="width:120px" readonly="readonly" value="${item.wareNm}" /></td>
								<td><input name="wareGg" type="text" class="pcl_i2" readonly="readonly"   value="${item.wareGg}" /></td>
								<td><input name="unitName" type="text" class="pcl_i2" readonly="readonly"  value="${item.unitName}" /></td>
								<td ><input name="edtqty" type="text" class="pcl_i2" value="${item.qty}" onchange="countAmt()"/></td>
								<td ><input id="sourceMainNo${s.index }" name="sourceMainNo"readonly="readonly" style="width:120px"   class="pcl_i2" value="${item.sourceMainNo}"/></td>
								<td style="display:none"><input id="beUnit${s.index }" name="beUnit" type="hidden" class="pcl_i2" value="${item.beUnit}" /></td>
								<td style="display: none"><input id="sourceMainId${s.index }" name="sourceMainId" type="hidden" class="pcl_i2" value="${item.sourceMainId}"/></td>
								<td style="display: none"><input id="sourceItemId${s.index }" name="sourceItemId" type="hidden" class="pcl_i2" value="${item.sourceItemId}"/></td>
								<td><a href="javascript:;"onclick="deleteChoose(this);" class="pcl_del">删除</a></td>
								</tr>
 								</c:forEach>
							</tbody>
	...
	<div id="wareCollarDlg" closed="true" class="easyui-dialog" style="width:500px; height:200px;" title="物料领用" iconCls="icon-edit">
     </div>
	...
	<script type="text/javascript">
		...
		function dialogSelectWare(){
			   var  stkId = $("#stkId").val();
		    	...
		    	$('#wareDlg').dialog({
		            ...
		            height: 400,
		            modal: true,
		            href: "<%=basePath %>/manager/dialogWareType?stkId="+stkId+"&p=1",
		            onClose: function(){
		            }
		        });
		    	$('#wareDlg').dialog('open');
		...
		function dialogCollarWare(){
			   collarWareList = new Array();
		    	$('#wareCollarDlg').dialog({
		            title: '物料领用选择',
		            iconCls:"icon-edit",
		            width: 800,
		            height: 400,
		            modal: true,
		            href: "<%=basePath %>/manager/toDialogCollarWarePage",
		            onClose: function(){
		            }
		        });
		    	$('#wareCollarDlg').dialog('open');
		  }
		function dialogOnDblClickRow(rowIndex, rowData)
		{
			var json = {};
			var wareList = new Array();
			var data = {
					itemId:rowData.item_id,
					mainId:rowData.main_id,
					billNo:rowData.bill_no,
					wareId:rowData.ware_id,
					wareNm:rowData.ware_nm,
					beUnitName:rowData.ware_dw,
					outQty:rowData.out_qty,
					beUnit:rowData.be_unit
			};
			wareList.push(data); 
			json ={
					list:wareList
			}
			callBackFunCollar(json); 
		}
		
		function callBackFunCollar(json){
			var size = json.list.length;
			if(size>0){
				for(var i=0;i<size;i++){
					setTabRowCollarData(json.list[i]);
				}
			}
		}
		 ...
		 function setTabRowData(json)
		 {
				...
				$("#more_list tbody").append(
						'<tr>'+
							...
							'<td style="display:none"><input id="edtprice'+rowIndex+'" name="edtprice" type="text" class="pcl_i2" value="' + price + '" onchange="countAmt()"/></td>'+
							'<td style="display:none"><input id="amt'+rowIndex+'" name="amt" type="text" class="pcl_i2" value="' + price + '" onchange="countPrice()"/></td>'+
		 ...
		 function setTabRowCollarData(json)
		 {
				 var itemId= json.itemId;
			     var mainId =	json.mainId;
				 var billNo =  json.billNo;
				 var wareId =  json.wareId;
				var wareNm = json.wareNm;
				var unitName = json.beUnitName;
				var qty=json.outQty;
				var beUnit=json.beUnit;
				
				$("#more_list_rela tbody").append(
						'<tr>'+
						'<td style="padding-left: 20px;text-align: left;display:none">'+
						'<input type="hidden" name="wareId" value = "'+wareId+'"/>'+
						'</td>'+
						'<td><input name="wareNm" type="text" class="pcl_i2" style="width:120px" readonly="readonly" value="'+wareNm+'" /></td>'+
						'<td><input name="wareGg" type="text" class="pcl_i2" readonly="readonly"   value="11" /></td>'+
						'<td><input name="unitName" type="text" class="pcl_i2" readonly="readonly"  value="'+unitName+'" /></td>'+
						'<td ><input name="edtqty" type="text" class="pcl_i2" value="'+qty+'" onchange="countAmt()"/></td>'+
						'<td ><input id="sourceMainNo" name="sourceMainNo"readonly="readonly" style="width:120px"  class="pcl_i2" value="'+billNo+'"/></td>'+
						'<td style="display:none"><input id="beUnit" name="beUnit" type="hidden" class="pcl_i2" value="'+beUnit+'" /></td>'+
						'<td style="display: none"><input id="sourceMainId" name="sourceMainId" type="hidden" class="pcl_i2" value="'+mainId+'"/></td>'+
						'<td style="display: none"><input id="sourceItemId" name="sourceItemId" type="hidden" class="pcl_i2" value="'+itemId+'"/></td>'+
						'<td><a href="javascript:;"onclick="deleteChoose(this);" class="pcl_del">删除</a></td>'+
						'</tr>'
					);
				countAmt();
