...
<html>
	...
	<body class="bg1">
		...
		<div class="src_box" style="display: none">
			<div class="src">
				...
				<table width="100%">
					<thead>
						<tr >							
							...
							<td align="center" style="font-size: 11px">收货数量</td>
							<td align="center" style="font-size: 11px">采购金额</td>
							<td align="center" style="font-size: 11px">平均单价</td>
						<tr >							
							<td align="center" style="font-size: 11px">合计</td>
							<td align="center" style="font-size: 11px"></td>
							<td align="center" style="font-size: 11px" id="totalFpQty"></td>
							<td align="center" style="font-size: 11px" id="totalShQty"></td>
	...
	function queryData(){
			...
			var path = "<%=basePath %>/web/stockWebRpt/wareInStat";
			...
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"waretype":waretype,"wareNm":wareNm,"stkId":0,"sdate":startDate,"edate":endDate,"page":pageIndex,"rows":20},
		        dataType: 'json',
		        async : false,
		        	...
		        	if(json.state){
		        		...
		        		if(size>0){
		        			...
		        			for(var i = 0;i < size-1; i++)
		        			{
								...
								text += "<td>" + amtformatter(json.rows[i].sumQty1) + "</td>";
								text += "<td>" + amtformatter(json.rows[i].sumAmt) + "</td>";
								var avg = parseFloat(json.rows[i].sumAmt)/parseFloat(json.rows[i].sumQty);
								text += "<td>" + numeral(avg).format("0,0.00") + "</td>";
		        				text = text + "</tr>";
		        			}
		        			$("#dataList").append(text);
		        			pageIndex++;
		        			var data = json.rows[size-1];
        					$("#totalFpQty").html(amtformatter(json.rows[i].sumAmt));
        					$("#totalShQty").html(amtformatter(json.rows[i].sumQty));
