	...
	<body>
		...
		<div class="center">
			...
			<div class="pcl_lib_out">
				<p class="pcl_title">领用出库发货单-${stkSend.voucherNo}-<c:if test="${stkSend.status eq 2 }"><span style="color: red">作废单</span></c:if><c:if test="${stkSend.status ne 2 }"><span style="color: green">正常单</span></c:if></p>
					...
					<div class="pcl_lfbox1">
						<table id="more_list">
							...
							<tbody id="chooselist">
								...
								<td>${item.wareGg}</td>
								<td>${item.unitName}</td>
								<td style="display:none"><input name="edtprice" type="text" class="pcl_i2" valuenone="${item.price}" readonly="readonly"/></td>
