...
<html>
		...
		<div class="sctb1" touchmove="queryData()">
			<div class="sctbw" >
				<table width="100%">
					<thead>
						<tr >							
							<td align="center" style="font-size: 11px">账号类型</td>
							<td align="center" style="font-size: 11px">账号</td>
							<td align="center" style="font-size: 11px">期初金额</td>
							<td align="center" style="font-size: 11px">本期增加</td>
							<td align="center" style="font-size: 11px">本期减少</td>
							<td align="center" style="font-size: 11px">期末余额</td>
						<tr >							
	...
	function queryData(){
			...
			var path = "<%=basePath %>/web/assetsWebRpt/cashDayStat";
			var token = $("#tmptoken").val();
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"sdate":startDate,"edate":endDate},
		        dataType: 'json',
		        async : false,
		        success: function (json) {
		        		...
		        		if(size>0){
		        			...
		        			{
		        				text +=  "<tr >";
								text += "<td>" + formatterType(json.rows[i].acc_type) + "</td>";
								text += "<td>" + json.rows[i].acc_no + "</td>";
								text += "<td>" + amtformatter(json.rows[i].qcamt) + "</td>";
								text += "<td>" + amtformatter(json.rows[i].inamt) + "</td>";
								text += "<td>" + amtformatter(json.rows[i].outamt) + "</td>";
								text += "<td>" + amtformatter(json.rows[i].qmamt) + "</td>";
		        				text = text + "</tr>";
		        			}
...
function formatterType(v){
	if(v == 0)return "现金";
	else if(v == 1)return "微信";
	else if(v == 2)return "支付宝";
	else if(v == 3)return "银行卡";
	else if(v == 4)return "无卡账号";
	else return "";
}
