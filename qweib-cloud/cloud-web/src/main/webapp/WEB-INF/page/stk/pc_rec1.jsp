	...
	<body>
		...
		<div class="box">
  			...
  			<input  type="hidden" name="accId" id="accId" value="0"/> 	
  			    <dl id="dl">
	        		...
	        		<dd>
	        		    <span class="title">剩余发票金额：</span><span style="color: red">${needRec1}</span>
	        		</dd>
	        		<dd>
	        		...
	        		<dd>
	        			...
	        			<input type="checkbox" id="preAmtChk" onclick="showPreAmtDic()"/><span style="font-size: 5px;color: blue">(勾选关联往来抵扣款单)</span>
	        		</dd>
	        		<div id="preAmtDiv" style="display: none">
	        		<dd>
	      				<span class="title">抵扣款单：</span>
	        			<input type="hidden" name="preId" id="preId"/>
	        			<input type="text" class="reg_input"  name="preNo" id="preNo"  readonly="readonly" placeholder="点击选择往来抵扣款单"  onclick="selectFinPreIn()"/>
						...
						<input type="radio" name="ysChk"  checked="checked" onclick="showPreAmtDic()"/>预收款单 <input name="ysChk" type="radio" id="yfkChk" onclick="showPreAmtDic()"/>应付款单
	        		</dd>
	        		<dd>
		...
		<script type="text/javascript">
			...
			function toSubmit(){
					...
					{
						$.messager.alert('消息','收款金额不能大于发票金额('+needRec+')!','info');
						return;
					}
					$.messager.confirm('确认', '是否确定保存?',function(r){
						if(r){
							var path = "manager/recProc";
							$.ajax({
						        url: path,
						        type: "POST",
			...
			function selectFinPreIn(){
				var url = "manager/toSelectFinPreInPage?proName=${cstName}";
				var title = "选择预收款单";
				if($("#yfkChk").is(":checked")== true){
					url = "manager/dialogPayPage?proName=${cstName}";
					title = "选择应付款单";
				}
		    ...
		    {
		    	var freeAmt = 0;
		    	if(rowData.freeAmt!=undefined&&rowData.freeAmt!=""){
					freeAmt = rowData.freeAmt;
				}
