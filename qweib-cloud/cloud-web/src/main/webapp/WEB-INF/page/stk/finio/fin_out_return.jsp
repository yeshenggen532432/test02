...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body onload="initGrid()">
		...
		<div>
			...
			<a class="easyui-linkbutton" iconCls="icon-search" href="javascript:showTotal();">应回款统计</a>
		...
		<script type="text/javascript">
			...
			function formatterSt3(val,row){
		      	...
		      	var ret ="";
		      	//alert(row.totalAmt+":"+row.payAmt);
		      	if(parseFloat(row.totalAmt)> parseFloat(row.payAmt)){
		      	ret = "<input style='width:60px;height:27px' type='button' value='收款' onclick='toFinReturn("+row.id+")'/>";
				ret =ret+"<input style='width:60px;height:27px' type='button' value='核销' onclick='toFreePay("+row.id+")'/>";
		   		 }

		      	if(row.payAmt > 0)
		      	ret = ret + "<input style='width:60px;height:27px' type='button' value='收款明细' onclick='showDetail("+row.id+")'/>";
		    ...
		    function toFinReturn(id){
				...
				parent.add('往来回款单','manager/finreturn?billId=' + id);
		    ...
		    function showDetail(id)
			{
				...
				parent.add('还款明细','manager/finaccio1?remarks=借款收回&billId=' + id);
			}
			...
			function onDblClickRow(rowIndex, rowData)
		    {
				...
				parent.closeWin('往来借出' + rowData.id);
		    	parent.add('往来借出' + rowData.id,'manager/showFinOutEdit?billId=' + rowData.id);
			...
			function amtformatter(v,row)
			{
			 ...
			 function showTotal()
				{
			    	parent.closeWin('应回款统计');
					parent.add('应回款统计','manager/toFinOutTotal');
				}
			...
			function freePay()
			{
				...
				$.messager.confirm('确认', msg, function(r) {
					if (r) {
						$.ajax( {
							url : "manager/updateFinOutFreeAmt",
