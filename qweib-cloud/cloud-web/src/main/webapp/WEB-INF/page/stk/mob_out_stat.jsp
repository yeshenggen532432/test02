...
<html>
		...
		<input  type="hidden" name="stkId" id="stkId" value="0"/>
			<table>
				<tr>
					...
					<td width="25%">
						<div class="dtbox">
							<a href="javascript:;" id="inType">销售类型</a>
							<img src="<%=basePath %>/resource/stkstyle/img/icon22.png"/>
						</div>
					</td>
				</tr>
		...
		<div class="sctb">
			<div class="sctbw">
				<table>
					<thead>
						<tr>							
							<td align="center" style = "WIDTH:40%;">产品名称</td>
							<td align="center" style = "WIDTH:15%;">单位</td>
							<td align="center" style = "WIDTH:20%;">发票数量</td>
							
							
							<td align="center" style = "WIDTH:25%;">发票金额</td>
						</tr>
					</thead>
					<tbody id="dataList">
						<tr>
	...
	<script type="text/javascript">
		...
		var a = $(window).width();//$(".sctbw table").width();
		$(".sctbw").width(a);
		//alert(a);
		queryStorage();
		//queryDict();
	...
	var endDate = $("#demo2").val();
			...
			var path = "stkOutSubStatList";
			var token = $("#tmptoken").val();
			var stkId = $("#stkId").val();
			var inType = $("#inType").text();
			if(inType=="销售类型"||inType=="全部类型")inType = "";
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"sdate":startDate,"edate":endDate,"stkId":stkId,"wareNm":searchKey,"xsTp":inType,"page":pageIndex,"rows":20},
		        dataType: 'json',
		        async : false,
		        success: function (json) {
		        	lock = true;
...
function chooseBill(trobj)
{
	...
	window.location.href='stkoutQueryDetail?token=' + token + '&sdate=' + startDate + '&edate=' + endDate + '&wareId=' + wareId +   '&stkId=' + stkId;
}
