...
<html>
	...
	<body class="bg1">
		...
		<div class="src_box">
			<div class="src">
				...
				<input id="khNm" type="text"   placeholder="往来单位" />
			</div>
		</div>
				...
				<table width="100%">
					<thead>
						<tr >							
							...
							<td align="center" style="font-size: 11px">应收金额</td>
	...
	function queryData(){
			...
			var path = "<%=basePath %>/web/orapWebRpt/qtysAmtStat";
			var token = $("#tmptoken").val();
			var khNm = $("#khNm").val();
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"sdate":startDate,"edate":endDate,"proName":khNm},
		        dataType: 'json',
		        async : false,
		        	...
		        	if(json.state){
		        		...
		        		if(size>0){
		        			for(var i = 0;i < size; i++)
		        			{
								...
								text += "<td>" + amtformatter(json.rows[i].needPay) + "</td>";
		        				text = text + "</tr>";
		        				//totalAmt = parseFloat(totalAmt)+parseFloat(json.rows[i].needPay);
		        			}
		        			$("#dataList").append(text);
		        			//$("#totalAmt").html(amtformatter(totalAmt));
		        		}else{
		        		}
		        	}
		        }
