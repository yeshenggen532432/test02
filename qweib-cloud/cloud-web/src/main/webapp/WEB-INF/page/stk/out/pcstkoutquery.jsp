...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body>
		...
		<div id="tb" style="padding:5px;height:auto">
			...
			发票日期: <input name="sdate" id="sdate"  onClick="WdatePicker();" style="width: 60px;"  readonly="readonly"/>
			<img onclick="WdatePicker({el:'sdate'})" src="resource/skin/datePicker.gif" width="16" height="22" align="absmiddle" style="cursor: pointer;"/>
			-
			<input name="edate" id="edate"  onClick="WdatePicker();" style="width: 60px;" value="${edate}" readonly="readonly"/>
	   ...
	   备注: <input name="remarks" id="remarks" style="width:120px;height: 20px;" />	  
	    显示商品信息:<input type="checkbox"  id="showWareCheck" name="showWareCheck" onclick="queryorder()" value="1"/>
			...
			<tag:permission name="发货明细查询" image="icon-search" onclick="javascript:showOutListQuery();"   buttonCode="stk.stkSend.senditems"></tag:permission>
		...
		<script type="text/javascript">
			...
			function formatterSt3(val,row){
				...
				if(row.sumOutQty!=0&&row.sumQty>=row.sumOutQty){
					ret = "<input style='width:60px;height:27px;display:${permission:checkUserFieldDisplay('stk.stkSend.senditems')}' type='button' value='发货明细' onclick='showOutList("+row.id+")'/>";
				}
				if(row.status == 0&&row.sumQty>row.sumOutQty){
					ret = ret + "<br/><input style='width:60px;height:27px;margin-top:2px;;display:${permission:checkUserFieldDisplay('stk.stkSend.send')}' type='button' value='发货' onclick='showCheck(\""+row.id+"\",\""+row.outType+"\",\""+row.billStatus+"\")'/>";
				}
				if((row.status == 0||row.status==1)&&row.sumOutQty!=0){
					ret = ret + "<br/><input style='width:60px;height:27px;margin-top:2px;;display:${permission:checkUserFieldDisplay('stk.stkSend.back')}' type='button' value='退货' onclick='showRtn("+row.id+")'/>";
				}
		      	var index= $("#datagrid").datagrid("getRowIndex",row);
		      	if(row.status == 0&&row.sumOutQty!=0){
		    ...
		    function showCheck(billId,outType,status)
			{
				...
				else if(outType == "报损出库")
		    		{
		    		parent.closeWin('报损发货开单');
		    		parent.add('报损发货开单','manager/showstkoutcheck?dataTp=1&billId=' + billId);
		    		}
		    	else
		    		{
