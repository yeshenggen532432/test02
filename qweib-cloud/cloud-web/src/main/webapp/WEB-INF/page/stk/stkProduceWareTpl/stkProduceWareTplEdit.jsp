...
<form action="<%=basePath %>/manager/stkProduceWareTpl/save" name="savefrm" id="savefrm" method="post">
										...
										<c:forEach items="${subList}" var="item" varStatus="s">
											<tr>
												<td style="padding-left: 20px;text-align: left;display: none">
													<input id="wareId${s.index }" type="hidden" name="subList[${s.index }].wareId" value = "${item.relaWareId}"/>
												</td>
												<td>
													<input id="wareNm${s.index }" name="subList[${s.index }].wareNm" readonly="readonly" type="text" style="width: 150px" class="pcl_i2" value="${item.relaWareNm}"/>
												</td>
												<td>
													<select id="beUnit${s.index }" name="subList[${s.index }].beUnit" style="width:50px" class="pcl_sel2">
														<option value="${item.relaMaxUnitCode}" <c:if test="${item.relaBeUnit == 'B'}"> selected</c:if>>${item.relaWareDw}</option>
														<option value="${item.relaMinUnitCode}" <c:if test="${item.relaBeUnit == 'S'}"> selected</c:if>>${item.relaMinUnit}</option>
													</select>
												</td>
												<td></td>
											</tr>
											<script>
												map.put('${item.relaWareId}','${item.relaWareNm}');
											</script>
										</c:forEach>
										</tbody>
									</table>
	...
	<div title="原料" style="padding:10px;">
										...
										<td>
											<select  id="relaWareId${s.index }" name="itemList[${s.index }].relaWareId" class="pcl_sel2">
												<c:forEach items="${subList}" var="sub">
													<option value="${sub.relaWareId}"}>${sub.relaWareNm}</option>
												</c:forEach>
											</select>
											<script>
												document.getElementById("relaWareId${s.index}").value='${item.relaWareId}';
											</script>
											<input type="hidden"  name="itemList[${s.index }].readersIds" value="${item.readersIds}" />
											<input type="hidden"  name="itemList[${s.index }].readersNms" value="${item.readersNms}" />
											<input id="relaWareNm${s.index }" type="hidden"  name="itemList[${s.index }].relaWareNm"  class="pcl_i2" value="${item.relaWareNm}" />
											<input type="hidden" id="relaUnitName${s.index }" name="itemList[${s.index }].relaUnitName" value="${item.relaUnitName}"/>
											<input type="hidden" id="relaBeUnit${s.index }" name="itemList[${s.index }].relaBeUnit" value="${item.relaBeUnit}"/>
										</td>
				...
				<a class="easyui-linkbutton" iconCls="icon-back" plain="true" href="javascript:history.back(-1);">返回</a>
...
<script type="text/javascript">
	...
	function resetTabRelaIndex(){
		...
		for(var i=0;i<trList.length;i++){
			var tdArr = trList.eq(i);
			var  name = $(tdArr).find("input[name$='wareId']").prop("name");
			...
			$(tdArr).find("input[name$='readersIds']").attr("name","itemList["+i+"].readersIds");
			$(tdArr).find("input[name$='readersNms']").attr("name","itemList["+i+"].readersNms");
			$(tdArr).find("input[name$='relaBeUnit']").attr("name","itemList["+i+"].relaBeUnit");
			$(tdArr).find("input[name$='relaUnitName']").attr("name","itemList["+i+"].relaUnitName");
	...
	function audit(){
		$.messager.confirm('确认', '是否确定审批?',function(r){
			if(r){
				$.ajax({
					url: "<%=basePath %>/manager/stkProduceWareTpl/audit",
					type: "POST",
					data : {"billId":billId},
					dataType: 'json',
					async : false,
	...
	function cancel(){
		$.messager.confirm('确认', '是否确定作废?',function(r){
			if(r){
				$.ajax({
					url: "<%=basePath %>/manager/stkProduceWareTpl/cancel",
					type: "POST",
					data : {"billId":billId},
					dataType: 'json',
					async : false,
