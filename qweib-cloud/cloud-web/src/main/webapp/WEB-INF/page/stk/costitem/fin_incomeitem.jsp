...
<html>
	...
	<input type="hidden" id="itemId" value=${itemId} />
		...
		<%--<a class="easyui-linkbutton" iconCls="icon-add"  href="javascript:addTypeClick();">增加科目类别</a>--%>
		...
		<div data-options="region:'west',split:true,title:'其它收入收款科目'"
			style="width:300px;padding-top: 5px;">
			<table id="costTypeId" class="easyui-datagrid" fit="true" singleSelect="true"
					url="manager/queryIncomeTypeList" title="" iconCls="icon-save" border="false" rownumbers="true"
		...
		<div id="wareTypeDiv" data-options="region:'center'" title="明细科目">
			<table id="itemgrid" class="easyui-datagrid" fit="true" singleSelect="true"
					url="manager/queryIncomeItemList" title="" iconCls="icon-save" border="false" rownumbers="true"
			 		fitColumns="true" >
					<thead>
					<tr>
				    <th field="id" width="80" align="center" hidden="true">
					...
					<th field="mark" width="150" align="center" formatter="formatterMark">
						默认类型
					</th>
		...
		<div id="dlg" closed="true" class="easyui-dialog" title="科目类别" style="width:400px;height:200px;padding:10px"
			data-options="
				iconCls: 'icon-save',
	    ...
	    <br>
	     设为默认:<select name="mark" id="mark">
	     				<option></option>
	     				<option value="1">核销项目</option>
	            </select>
	    <br>
		...
		<script type="text/javascript">
			...
			function editCostItem()
			{
				...
				var mark = $("#mark").val();
				$.ajax( {
					url : "manager/saveFinIncomeItem",
					data : "id=" + id + '&itemName=' + itemName + '&mark='+mark+'&remarks=' + remarks +'&typeId=' + typeId,
					type : "post",
					success : function(json) {
						if (json.state) {
							showMsg("保存成功");
			...
			function formatterSt4(val,row){
				...
				if(row.systemId==""||row.systemId==null){
					ret = "<input style='width:60px;height:27px' type='button' value='修改' onclick='toEditItem("+row.id + ")'/>"
							+ "<input style='width:60px;height:27px' type='button' value='删除' onclick='deleteCostItem("+row.id+")'/>"
					;
				}
	      	        return ret;
			...
			function toEditType(typeId)
			{
				...
				var typeName = "";
				$.ajax( {
					url : "manager/queryIncomeTypeById",
					data : "typeId=" + typeId,
					type : "post",
					success : function(json) {
			...
			function toEditItem(itemId)
			{
				...
				var remarks = "";
				$.ajax( {
					url : "manager/queryIncomeItemById",
					data : "itemId=" + itemId,
					type : "post",
					success : function(json) {
			...
			{
				if(!confirm('是否确认删除该明细科目？'))return;
				$.ajax( {
					url : "manager/deleteIncomeItem",
					data : "id=" + itemId,
					type : "post",
					success : function(json) {
			...
			function querycosttype(){
				var path = "manager/queryIncomeTypeList";
			...
			function queryItem(typeId){
				$("#itemgrid").datagrid('load',{
					url:"manager/queryIncomeItemList",
					typeId:typeId
				});
			}
			...
			function addTemplate(){
				...
				parent.add('项目参考数据','manager/toFinIncomeItemTemplate');
			}
