...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body onload="initGrid()">
		<table id="datagrid"  fit="true" singleSelect="true"
			url="manager/stkInSubPage?dataTp=${dataTp }&sdate=${sdate}&timeType=${timeType }&edate=${edate}&stkId=${stkId}&wareId=${wareId}&proName=${proName}&memberNm=${memberNm}&inType=${inType}&proId=${proId}"  iconCls="icon-save" border="false"
		...
		<script type="text/javascript">
		   ...
		   function initGrid()
		   {
			...
			function onDblClickRow(rowIndex, rowData)
		    {
				...
				if("采购退货".indexOf(billName)!=-1){
					parent.closeWin('采购退货详细' + rowData.billId);
					parent.add('采购退货详细' + rowData.billId,'manager/showstkin?dataTp=1&billId=' + rowData.billId);
				}
			...
			function amtformatterPrice(v,row)
			{
				var outAmt = row.inAmt;
				var outQty = row.inQty;
				...
				if(outAmt=="0E-7"||outQty=="0E-7"){
					return "0.00";
				}
				var price = parseFloat(outAmt)/parseFloat(outQty);
				if (row != null) {
