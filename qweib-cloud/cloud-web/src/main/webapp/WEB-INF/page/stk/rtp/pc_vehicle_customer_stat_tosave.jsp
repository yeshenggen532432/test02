	...
	<body>
		...
		<div id="tb" style="padding:5px;height:auto">
		   ...
		   时间: <input name="stime" id="sdate"  onClick="WdatePicker();" style="width: 100px;" value="${sdate}" readonly="readonly"/>
	         ...
	         车牌号: <input name="vehNo" id="vehNo" value="${vehNo }"  style="width:120px;height: 20px;" />
		 	 送货人: <input name="driverName" id="driverName" value="${driverName }" style="width:120px;height: 20px;"/>
			...
			<form action="manager/saveVehicleCustomerStat" name="saveVehicleCustomerStatfrm" id="saveVehicleCustomerStatfrm" method="post">
			<input type="hidden" name="rptType" value="1"/>
			报表标题：<input name="rptTitle" value="${sdate}-${edate}车辆配送客户统计表" size="80"/>
			<br/>
			备&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;注：
			<textarea name="remark" rows="5" cols="90">报表参数--时间:${sdate}-${edate},车牌号:${vehNo },送货人:${driverName },品名:${wareNm},销售类型:${billName}
			</textarea>
			<br/>
			是否同步更新价格：
			...
			<td>
			<input readonly="readonly" name="items[${s.index}].driverName" value="${d.vehNo }"/>
			</td> 
			<td>
			...
			<td>
			<input name="items[${s.index}].unitAmt" onchange="changSumAmt(this,${s.index})" id="items[${s.index}].unitAmt" value="${d.tranAmt }"/>
			</td> 
			<td>
		...
		<script type="text/javascript">
	     ...
	     calMny();
			 function updateSubmit(){
				 $("#saveVehicleCustomerStatfrm").form('submit',{
				...
				//document.getElementById("saveVehicleCustomerStatfrm").submit();
			   }
		</script>
	</body>
</html>
