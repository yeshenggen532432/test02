...
<html>
	...
	<body class="bg1">
		...
		<div class="src_box">
			<div class="src" >
				...
				<input id="khNm" type="text"   placeholder="报销人" />
			</div>
		</div>
		...
		<div class="sctb1" touchmove="queryData()">
			<div class="sctbw" >
				<table width="100%">
					<thead>
						<tr >							
							<td align="center" style="font-size: 11px">单号</td>
							<td align="center" style="font-size: 11px">费用日期</td>
							<td align="center" style="font-size: 11px">报销人员</td>
							<td align="center" style="font-size: 11px">部门</td>
							<td align="center" style="font-size: 11px">报销金额</td>
							<td align="center" style="font-size: 11px">备注</td>
	...
	function queryData(){
			...
			var path = "<%=basePath %>/web/orapWebRpt/dzfCostAmtStat";
			var token = $("#tmptoken").val();
			var khNm = $("#khNm").val();
			$.ajax({
		        url: path,
		        type: "POST",
		        data : {"token":token,"sdate":startDate,"edate":endDate,"proName":khNm,"page":pageIndex,"rows":20},
		        dataType: 'json',
		        async : false,
		        	...
		        	if(json.state){
		        		...
		        		if(size>0){
		        			for(var i = 0;i < size; i++)
		        			{
								...
								text += "<td>" + json.rows[i].costTimeStr + "</td>";
								text += "<td>" + json.rows[i].proName + "</td>";
								text += "<td>" + json.rows[i].branchName + "</td>";
								text += "<td>" + amtformatter(json.rows[i].totalAmt) + "</td>";
        					...
        					//$("#totalAmt").html(amtformatter(json.rows[i].totalAmt));
