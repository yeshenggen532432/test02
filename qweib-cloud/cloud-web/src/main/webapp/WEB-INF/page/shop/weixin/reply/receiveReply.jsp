...
<script type="text/javascript">
    $(function(){
        ...
        toValidate();
        //选择单选框
        var receiveStatus="${config.receiveStatus}";
        if("${config.id}"==""){
            var weixinConfigId=0;
        }else{
            var weixinConfigId="${config.id}";
        }
        var getWeixinConfig_url="/manager/weiXinReply/getReceiveReplyText?weixinConfigId="+weixinConfigId;
        var receiveTex="";
        ...
        if(msg.responseText==""){
            ...
            var receiveReplyTextMap=json.receiveReplyTextMap;
            if(json.state){
                var receiveImageId="${config.receiveImageId}";
                var radioText=$("#radioText");
                var radioImage=$("#radioImage");
                if(receiveStatus == "0"){
                    radioText.prop("checked","checked");
                    replyContentType();
                    $("#replyContentTextarea").val( receiveReplyTextMap.receive_text);
                }else{
                    if(receiveImageId == "" || receiveImageId == null){
                        radioText.prop("checked","checked");
                        replyContentType();
                        $("#replyContentTextarea").val( receiveReplyTextMap.receive_text);
                    }else{
                        ...
                        replyContentType();
                        var getImage_url="/manager/weiXinReply/getReceiveImageUrl?receiveImageId="+receiveImageId;
                        ...
                        $("#buttonImagID").html(receiveImageId);
                    }
                }
            }else{
                alert("查询回复内容失败！");
        ...
        if("${config.id}"==""){
            ...
            switch(ContentType){
                case "text":
                    ...
                    var saveReply_url="/manager/weiXinReply/updateReceiveReply";
