...
<table id="datagrid" class="easyui-datagrid" fit="true" singleSelect="false"
       url="manager/memberPage?dataTp=${dataTp }&isDel=0" title="成员列表" iconCls="icon-save" border="false"
    ...
    <tr>
        ...
        <th field="memberId" width="40" align="center">
            ID
        </th>
        ...
        <th field="roleNames" width="110" align="center" formatter="formatRole">
            角色列表
        </th>
        <th field="memberCompany" width="150" align="center" formatter="formatAuthority">
            员工权限
        </th>
        <th field="branchName" width="80" align="center">
            部门
        </th>
        <th field="isLead" width="50" align="center" formatter="formatterSt">
            是否领导
        </th>
        <th field="isAdmin" width="60" align="center" formatter="formatterSt1">
            是否管理员
        </th>
        ...
        <th field="oper2" width="80" align="center" formatter="formatterSt6">
            加密锁操作
        </th>
        <th field="upload" width="80" align="center" formatter="formatterSt7">
            位置上传状态
        </th>
        <th field="min" width="50" align="center" formatter="formatterSt8">
            上传间隔
        </th>
        <th field="oper3" width="50" align="center" formatter="formatterSt9">
            上传操作
        </th>
        <th field="memUpload" width="110" align="center" formatter="formatterSt10">
            业务员设置上传方式
        </th>
    </tr>
    </thead>
...
<div id="tb" style="padding:5px;height:auto">
    ...
    <c:if test="${empty info.datasource}">
        公司: <input name="memberCompany" id="memberCompany" style="width:120px;height: 20px;"
...
<div id="treeDiv" class="easyui-window" style="width:300px;height:400px;"
    ...
    <form name="rolemenufrm" id="rolemenufrm" method="post">
        ...
        <div id="divHYGL" class="menuTree" data-options="region:'north'"
             style="width: 280px;height:330px;overflow: auto;padding-left: 5px;">
...
<script type="text/javascript">
    ...
    function toaddmember() {
        location.href = "${base}/manager/toopermember";
    }
    ...
    function formatterSt4(val, row) {
        if (val) {
            if (val == '1') {
                if (dataTp == '3') {
                    return "<input style='width:60px;height:27px' type='button' value='待绑定' onclick='updateUnId(this, " + row.memberId + ",1)' disabled='disabled'/>";
                } else {
                    return "<input style='width:60px;height:27px' type='button' value='待绑定' onclick='updateUnId(this, " + row.memberId + ",1)'/>";
                }
            } else {
                if (dataTp == '3') {
                    return "<input style='width:60px;height:27px' type='button' value='绑定' onclick='updateUnId(this, " + row.memberId + ",2)' disabled='disabled'/>";
                } else {
                    return "<input style='width:60px;height:27px' type='button' value='绑定' onclick='updateUnId(this, " + row.memberId + ",2)'/>";
                }
            }
        } else {
            if (dataTp == '3') {
                return "<input style='width:60px;height:27px' type='button' value='没绑定' onclick='updateUnId(this, " + row.memberId + ",3)' disabled='disabled'/>";
            } else {
                return "<input style='width:60px;height:27px' type='button' value='没绑定' onclick='updateUnId(this, " + row.memberId + ",3)'/>";
            }
        }
    }
    ...
    function formatAuthority(value, row) {
        return '<button style="min-width:60px;min-height:27px;font-size:12px;margin-left:5px;" onclick="queryAuthority(\'' + row.memberId + '\', 1)">查看菜单</button>' +
            '<button style="min-width:60px;min-height:27px;font-size:12px;margin-left:5px;" onclick="queryAuthority(\'' + row.memberId + '\', 2)">查看应用</button>';
    }

    function queryAuthority(memberId, menuType) {
        window.parent.showMemberAuthority(memberId, menuType);
    }
