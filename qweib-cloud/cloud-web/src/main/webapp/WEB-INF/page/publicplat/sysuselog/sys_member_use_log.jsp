...
<html xmlns="http://www.w3.org/1999/xhtml">
	...
	<body onload="query()">
		<table id="datagrid" class="easyui-datagrid" fit="true" singleSelect="true"
			url="manager/sysMemberUseLogPage"  border="false"
			...
			<thead>
				<tr>
					...
					<th field="fd_member_nm" width="150" align="center">
						用户
					</th>
		...
		时间: <input name="stime" id="sdate"  onClick="WdatePicker();" style="width: 100px;" value="${sdate}" readonly="readonly"/>
			...
			用户名称: <input name="fdMemberNm" id="fdMemberNm" style="width:156px;height: 20px;" onkeydown="toQuery(event);"/>
			<a class="easyui-linkbutton" iconCls="icon-search" href="javascript:query();">查询</a>
			<a class="easyui-linkbutton" iconCls="icon-search" href="javascript:show();">企业行为分析明细</a>
		</div>
		...
		<script type="text/javascript">
			...
			function query(){
				$("#datagrid").datagrid('load',{
					url:"manager/sysMemberUseLogPage",
					sdate:$("#sdate").val(),
					edate:$("#edate").val(),
			...
			function show(){
				...
				var fdMemberNm=$("#fdMemberNm").val();
		    	parent.closeWin('企业行为详细');
		    	parent.add('企业行为详细','manager/querysysuselogdetail?sdate='+sdate+'&edate='+edate+'&fdCompanyNm='+fdCompanyNm+'&fdMemberNm='+fdMemberNm+'');
			}
			function onDblClickRow(rowIndex, rowData){
				var sdate=$("#sdate").val();
				var edate=$("#edate").val();
				var fdCompanyNm=rowData.fd_company_nm;
				var fdMemberNm=rowData.fd_member_nm;
		    	parent.closeWin('企业行为详细');
		    	parent.add('企业行为详细','manager/querysysuselogdetail?sdate='+sdate+'&edate='+edate+'&fdCompanyNm='+fdCompanyNm+'&fdMemberNm='+fdMemberNm+'');
			}
			//回车查询
