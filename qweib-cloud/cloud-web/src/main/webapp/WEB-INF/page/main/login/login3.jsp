...
<head>
	...
	<link rel="stylesheet" href="/static/css/login.css?v=20190902">
		...
		<div class="v_cont">
			<form id="loginFrm" action="${base }/manager/dologin" method="post">
				...
				<ul>
					...
					<c:if test="${captcha ne null && captcha}">
						<li class="animate2" id="captcha-container" style="display: flex;">
							<input maxlength="4" minlength="4" id="captcha" name="captcha" style="width: 60%;"
								   placeholder="验证码" autocomplete="off"
								   onkeydown="tologinSubmit(event)">
							<img onclick="refreshCaptcha()" id="captcha-image" src="/captcha?h=35"
								 style="margin-left: 20px;"/>
						</li>
					</c:if>
					<li class="animate4 bounceIn s_rem">
						<input class="" type="checkbox" name="rememberMe" id="rememberMe" value="1"/>记住我
...
<script type="text/javascript" src="${base }/static/qwb/plugins/cookies.js"></script>
...
<script type="text/javascript" src="${base }/resource/showMsg.js?v=20190902"></script>
<script>
	//回车登录
	...
	function refreshCaptcha() {
		$('#captcha-image').attr('src', '/captcha?h=35&r=' + Math.random());
	}

	function validateCaptcha(callback) {
		<c:if test="${captcha eq null || !captcha }">
	...
	window.onload = function () {
		...
		$("#loginError").html(getMsg("${showMsg}", "${extra}"));
		if ("${showMsg}" != "") {
			$(".s_error").show();
			setTimeout(function () {
				$(".s_error").hide();
	...
	function getCookie() {
		...
		if (arr && arr.length > 0) {
			var username = arr[0]
			if (username) {
				document.getElementsByName("usrNo")[0].value = username;
			}
		}
		if (arr && arr.length > 1) {
			document.getElementsByName("tempPwd")[0].value = arr[1];
		}
