    ...
    <div class="layout-content">
        <div class="layui-card forget-box forget-main">
            ...
            <div class="layui-card-body">
                <div class="form form-horizontal" qwb-role="validator">
                    <div class="form-group">
                        <label class="control-label col-xs-6">姓&nbsp;&nbsp;&nbsp;名</label>
                        <div class="col-xs-18">
                            <input qwb-role="textbox" disabled qwb-model="name" value="${memberName}">
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-xs-6">手机号</label>
                        <div class="col-xs-18">
                            <input qwb-role="textbox" disabled qwb-model="mobile" value="${memberMobile}"/>
                        </div>
                    </div>
                    <div class="form-group">
                        <label class="control-label col-xs-6">验证码</label>
                        <div class="col-xs-9">
                            <input id="code" maxlength="6" qwb-validate="required" qwb-role="textbox" qwb-model="code"/>
                        </div>
                        <div class="col-xs-9">
                            <button qwb-role="button" type="button" onclick="getCode()" id="btn-send"
...
<script type="text/javascript">
    ...
    function getCode() {
        ...
        waiting = true;
        qwb.ui.get('#btn-send').enable(false);
        $('#btn-send').text('发送中...');
        $.ajax({
            ...
            success: function (response) {
                ...
                if (response.code === 200) {
                    ...
                    sent = false;
                    qwb.ui.error(response.message);
                    $('#btn-send').text('重新发送');
                    qwb.ui.get('#btn-send').enable(true);
                }
            },
            error: function () {
                waiting = false;
    ...
    function submit() {
        if (!sent) {
            return qwb.ui.error('请先获取验证码');
        }
        var validate = qwb.ui.get('.form').validate();
        if (!validate) {
            return
        }
        var data = qwb.ui.bind('.form');
        if (data.code.length < 6) {
            return qwb.ui.error('请输入6位验证码');
        }
        qwb.ui.loading();
        $.ajax({
