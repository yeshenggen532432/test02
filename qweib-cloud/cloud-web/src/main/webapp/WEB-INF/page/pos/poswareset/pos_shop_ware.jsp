...
<div data-options="region:'west',split:true,title:'商品分类树'"
    ...
    <ul id="waretypetree" class="easyui-tree"
        data-options="url:'',animate:true,dnd:true,onClick: function(node){
...
<div id="departDiv" data-options="region:'center'" title="门店销售商品">
    ...
    <div id="tb" style="padding:5px;height:auto">
        连锁店:
        <tag:select id="shopName" name="shopName" displayKey="shop_no" onchange="changeShop();" displayValue="shop_name" tableName="pos_shopinfo"/>
        ...
        <a class="easyui-linkbutton" iconCls="icon-search" plain="true"  href="javascript:queryShopWare();">查询</a>
        <a class="easyui-linkbutton" iconCls="icon-edit" plain="true"  href="javascript:addWares();">上架</a>
        <a class="easyui-linkbutton" iconCls="icon-edit" plain="true"  href="javascript:deleteWare();">下架</a>
...
<div id="w" class="easyui-window" title="编辑线下商品分类" data-options="iconCls:'icon-save',closed:true" style="width:500px;height:400px;padding:10px;">
    ...
    <ul id="waretypeTreeWindow" class="easyui-tree" data-options="url:'manager/pos/shopWaretypes',animate:true,checkbox:true"></ul>
</div>
<script type="text/javascript">
    ...
    function loadWaretype() {
        $('#waretypetree').tree({
            url: 'manager/pos/syswaretypes?shopNo=' + $("#shopName").val()
        });
    ...
    function initGrid() {
        ...
        $("#datagrid").datagrid({
            ...
            columns: [[
                ...
                {field: 'isSale', title: '上架状态', width: 90,formatter:formatPutOn},
    ...
    function queryShopWare(wareType){
        ...
        $('#datagrid').datagrid({
            url:"manager/pos/queryPosShopWare1",
            queryParams:{
                shopNo:shopNo1,
                wareType:wareType,
                wareNm:$("#wareNm").val(),
    ...
    function addWares()
    {
        ...
        for(var i=0;i<rows.length;i++){
            if(ids.length == 0)ids = rows[i].wareId;
            else
        ...
        $.ajax( {
            url : "manager/pos/addBatShopWare1",
            data : "shopNo=" + shopNo+"&wareStr=" + ids,
            type : "post",
            success : function(json) {
                if (json.state) {
                    //if(ope == 1)
    ...
    function getChecked(){
        $("#w").window('open');
        $('#waretypeTreeWindow').tree({
            url: 'manager/pos/shopWaretypes?shopNo=' + $("#shopName").val()
        });
    ...
    function savepower(){
        ...
        $.ajax({
            url : "manager/pos/updateShopWaretype",
            type : "post",
            data : {shopNo:shopNo,"ids":cIds,"ids1":iIds},
            success : function(json) {
                if(json.state){
                    alert("修改成功");
                    $("#w").window('close');
