        ...
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid"
                 uglcw-options="
                    ...
                    pageable: true,
                    url: 'manager/stkDiningRecMast/page',
                    criteria: '.query',
                    aggregate:[ {field: 'sumAmt', aggregate: 'sum'}],
                     loadFilter: {
                        ...
                        if (response && response.rows.length > 0) {
                             $.map(response.rows, function(item){
                                aggregate.sumAmt += item.sumAmt||0;
                             })
                        }
                        return aggregate;
                      }
                   ...
                   ">
                <div data-field="billNo" uglcw-options="width:100">收款单号</div>
                <div data-field="orderNo" uglcw-options="width:100">订单号</div>
                <div data-field="proName" uglcw-options="width:160">往来对象</div>
                <div data-field="recTime" uglcw-options="width:100">收款时间</div>
                <div data-field="staff" uglcw-options="width:100">收银人员</div>
                <div data-field="accTypeStr" uglcw-options="width:100">收款类型</div>
                <div data-field="sumAmt"
                     uglcw-options="width:100,align:'center', aggregates: ['sum'], footerTemplate: '\\#= data.sumAmt.toFixed(2)\\#'">
                    收款金额
                </div>
                ...
                <div data-field="diningOrderNo" uglcw-options="width:100">餐饮订单号</div>
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function cancelRec(id) {
        uglcw.ui.confirm('是否确认作废收款单？', function () {
            uglcw.ui.loading();
            $.ajax({
                url: "${base}manager/stkDiningRecMast/cancelStatus",
                type: "post",
                data: {
                    billId: id
                },
        ...
        var rtn = uglcw.ui.Modal.open({
            ...
            yes: function (container) {
                ...
                uglcw.ui.confirm('您确认入账吗', function () {
                    uglcw.ui.loading();
                    $.ajax({
                        url: '${base}manager/stkDiningRecMast/saveRecMast',
                        data: {
                            billId: billId,
                            accId: data.accId,
    ...
    function showDetail(row) {
        uglcw.ui.openTab('餐饮收款记录信息', '${base}manager/stkDiningRecMast/show?billId=' + row.id);
    }
</script>
</body>
</html>
