				...
				<div class="layui-card-header btn-group" style="height: 32px;">
					...
					<div class="bill-info">
						<span class="no" style="color: green;">
							领用出库发货单-${stkSend.voucherNo}-<c:if test="${stkSend.status eq 2 }"><span style="color: red">作废单</span></c:if><c:if test="${stkSend.status ne 2 }"><span style="color: green">正常单</span></c:if>
						</span>
							...
							<div class="col-xs-4">
								<input uglcw-role="gridselector" value="${stkSend.billNo}"
									   uglcw-options="click: function(){
											uglcw.ui.openTab('领用出库单据信息','${base}manager/showstkout?dataTp=1&billId=${stkSend.outId}');
									   }">
							</div>
...
<script>
	...
	function toPrint(id)
	{
		uglcw.ui.openTab('领用出库发货单打印', '${base}manager/showstksendprint?dataTp=1&billId=' + id);
	}
