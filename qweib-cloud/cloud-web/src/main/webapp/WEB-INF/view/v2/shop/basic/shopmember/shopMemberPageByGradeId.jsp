				...
				<div class="layui-card-body full">
					<div id="grid" uglcw-role="grid"
						 uglcw-options="
							...
							checkbox:true,
							pageable: true,
                    		url: '${base}manager/shopMember/shopMemberPageByGradeId?gradeId=${gradeId}',
                    		criteria: '.query',
                    		pageSize: 20,
                    	">
						<div data-field="name" uglcw-options="width:100">会员名称</div>
						<div data-field="mobile" uglcw-options="width:110">电话</div>
						<div data-field="defaultAddress" uglcw-options="width:200,tooltip:true">地址</div>
...
<script id="oper" type="text/x-uglcw-template">
	# if(data.status=='-1'){ #
	<button onclick="javascript:updateStatus(#= data.id#,1);" class="k-button k-info">通过</button>
	# }else if(data.status=='0'){ #
	<button onclick="javascript:updateStatus(#= data.id#,1);" class="k-button k-info">启用</button>
	# }else if(data.status=='1'){ #
	<button onclick="javascript:updateStatus(#= data.id#,0);" class="k-button k-info">不启用</button>
	# } #
