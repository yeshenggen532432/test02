                        ...
                        <div class="form-group" style="margin-bottom: 5px!important;">
                            <div class="col-xs-3">
                                <input uglcw-model="checkTimeStr" uglcw-role="datetimepicker"
                                       ...
                                       value="${checkTime}" placeholder="清算时间">
                            </div>
                            <div class="col-xs-3" >
                            ...
                            <div class="col-xs-3 ">
                                <input value="${staff}" placeholder="清盘人员" uglcw-model="staff,empId"
                            ...
                            <div style="font-size: 11px">
                                清算表保存审批后，库存结存信息将以清算信息为准，并以清算数及金额成本为准参与后续库存运算!
                            </div>
                        </div>
                    </div>
                </div>
            ...
            <div class="layui-card">
                <div class="layui-card-body full">
                    ...
                    ">
                        ...
                        <div data-field="checkQty" uglcw-options="width:90">清算数</div>
                        <div data-field="qty"
                             uglcw-options="width:120,hidden:true">清算库存(大)
                        </div>
                        <div data-field="_qty"
                             uglcw-options="width:120,
                              attributes:{class: 'uglcw-cell-speedy k-dirty-cell maxQtyFlag'},
                              editable: true, schema:{type: 'number',decimals:10}">清算库存(大)
                        </div>
                        ...
                        <div data-field="maxAmt"
                             uglcw-options="width:120">清算实际成本(大)
                        </div>
                        ...
                        <div data-field="minAmt"
                             uglcw-options="width:120">清算实际成本(小)
                        </div>
...
<script type="text/x-uglcw-template" id="toolbar">
    ...
    <div class="k-button" >
        <input type="checkbox" uglcw-role="checkbox" onclick="fiterData(0)"  uglcw-model="lessZeroChk"
               uglcw-options="type:'number'"
               class="k-checkbox" id="lessZeroChk"/>
        <label style="margin-bottom: 0;" class="k-checkbox-label"  for="lessZeroChk">过滤单位成本<=0数据</label>
    </div>

    <span style="color: red;">清算库存(大)+清算库存(小)=总清算数量</span>
...
<script>
    ...
    function drageSaveStk() {
        ...
        uglcw.ui.confirm(msg, function () {
            $.ajax({
                url: '${base}manager/drageStkRevise',
                type: 'post',
                data: data,
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (response.state) {
                        uglcw.ui.success(response.msg);
                        setTimeout(function () {
                            uglcw.ui.replaceCurrentTab('库存清算详情', '${base}manager/showstkrevise?billId='+billId);
                        }, 1000)
                    } else {
                        uglcw.ui.error(response.msg);
                    }
    ...
    function submitStk() {
        ...
        var data = uglcw.ui.bind('.query');
        if (data.billId > 0) {
            return uglcw.ui.warning('清算单不能修改');
        }
        var list = uglcw.ui.get('#grid').bind();
        ...
        uglcw.ui.confirm('保存后将不能修改，是否确定保存并审批,注意：清算表保存审批后，库存结存信息将以清算信息为准，并以清算数及金额为准，且后续库存运算!";', function () {
            uglcw.ui.loading();
            $.ajax({
                url: '${base}manager/addStkRevise',
                type: 'post',
                data: data,
                success: function (response) {
                    uglcw.ui.loaded();
                    if (response.state) {
                        uglcw.ui.success(response.msg);
                        setTimeout(function () {
                            uglcw.ui.replaceCurrentTab('库存清算详情', '${base}manager/showstkrevise?billId=' + response.id);
                        }, 1000)
                    } else {
                        uglcw.ui.error(response.msg);
                    }
