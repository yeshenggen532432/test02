...
<div id="printcontent" style="width:100%;text-align: center">
    <table style="padding-left:30px;width: 95%">
        <tr>
            <td width="30%"></td>
            <td style="font-size: 18px;font-weight: bold;text-align: center">业务员销售统计表</td>
    ...
    <table style="padding-left: 30px;width: 95%;border-collapse: collapse;">
        ...
        <tr>
            ...
            <td style="width:140px;font-weight:bold;border:1px solid #000000;">业务员</td>
            <td style="width:140px;font-weight:bold;border:1px solid #000000;">部门</td>
            <c:if test="${showWareCheck ne 1}">
            ...
            <td style="width:100px;text-align: center;border:1px solid #000000;">回款金额</td>
            <td style="width:100px;text-align: center;border:1px solid #000000;">未回款金额</td>
        </tr>
        </thead>
        <tbody id="chooselist">
        </tbody>
        <c:forEach items="${datas}" var="item" varStatus="s">
            <tr>
                ...
                <td style="border:1px solid #999999;">${item.staff}</td>
                <td style="border:1px solid #999999;">${item.branchName}</td>
                <c:if test="${showWareCheck ne 1}">
                ...
                <c:if test="${showWareCheck ne 1}">
                    ...
                    <td class="products" style="border:1px solid #999999;text-align: right;padding-right: 10px">${item.minPrice}</td>
                </c:if>
                ...
                <td style="border:1px solid #999999;text-align: right;padding-right: 10px">
                    <fmt:formatNumber value="${item.recAmt}" pattern="#,###.##"/>
                </td>
                <td style="border:1px solid #999999;text-align: right;padding-right: 10px">
                    <fmt:formatNumber value="${item.needRec}" pattern="#,###.##"/>
                </td>
            </tr>
        </c:forEach>
    </table>
...
<script>
    ...
    function filterField(){
        if('${showWareCheck}'==1){
            $(".products").hide();
        }else  if('${showWareCheck}'==0){
            $(".products").show();
        }else if('${showWareCheck}'==-1){
            $(".products").show();
            $(".xsTp").hide();
            //$(".xsTp").show();
        }
