        ...
        <div style="padding-bottom: 50px;">
            <i-form ref="form" :label-width="120" :model="shopTourPlan" :rules="rules">
                ...
                <form-item  class="main-item"  label="结束时间到,人数未满订单" prop="count">
                    <input type="radio" v-model="shopTourPlan.endTimeSuccess" value="0"/>无效
                    <input type="radio" v-model="shopTourPlan.endTimeSuccess"  value="1"/>有效
                    <span style="padding-left: 10px;color: red;">
                        <span v-if="shopTourPlan.endTimeSuccess==0">选择“无效”时，拼团时间结束后，拼团订单自动取消并自动退款用户。</span>
                        <span v-if="shopTourPlan.endTimeSuccess==1">选择“有效”时，已买的订单自动生效买单。</span>

                    </span>
                </form-item>
                <form-item  class="main-item"  label="参团人数" prop="count">
                    <input-number :disabled="shopTourPlan.status == 1" :min="2" :max="100" :active-change="false"
        ...
        vm = new Vue({
            ...
            methods: {
                load: function () {
                    ...
                    vm.$Spin.show();
                    $.ajax({
                        url: '${base}manager/shopTourPlan/get?id=' + vm.shopTourPlan.id,
                        type: 'get',
                ...
                save: function () {
                    ...
                    vm.$refs.form.validate(function (v) {
                        if (v) {
                            ...
                            $.ajax({
                                url: '${base}manager/shopTourPlan/save',
                                type: 'post',
                                ...
                                success: function (response) {
                                    if (response.code === 200) {
                                        ...
                                        uglcw.io.emit('shop_tour_plan_chage',1);
                                    } else {
                                        vm.$Message.error(response.message);
                                    }
                                    vm.$Spin.hide();
