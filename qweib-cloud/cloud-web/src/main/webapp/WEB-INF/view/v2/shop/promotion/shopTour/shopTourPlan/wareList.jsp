            ...
            <div class="layui-card-body full">
                <div id="grid" uglcw-role="grid"
                     uglcw-options="
							...
							autoAppendRow: false,
							pageable: false,
                    		url: '${base}/manager/shopTourWare/list?planId=${plan.id}',
                    		criteria: '.query',
                    		loadFilter: {
                    	...
                    	">
                    <%--<div data-field="id" uglcw-options="width:150,hidden:true">id</div>
                    <div data-field="status" uglcw-options="width:150,hidden:true">status</div>
                    <div data-field="postAmt" uglcw-options="width:150,hidden:true">postAmt</div>
                    <div data-field="description" uglcw-options="width:150,hidden:true">description</div>
                    <div data-field="isFreePost" uglcw-options="width:150,hidden:true">isFreePost</div>
                    <div data-field="stockQty" uglcw-options="width:150,hidden:true">stockQty</div>--%>
                    <div data-field="name" uglcw-options="width:150,schema:{editable: true}">拼团名称</div>
                    ...
                    <div data-field="price"
                         uglcw-options="width: 60, schema:{editable: true, type: 'number',validation:{required:true, min:0.1}},
                             editor: function(container, options){
                             var model = options.model;
                             if(model.status!=0){
                    ...
                    <div data-field="orderCd"
                       ...
                       <%-- <div data-field="endTimeSuccess"
                             ...
                             template: uglcw.util.template($('#endTimeSuccess_tpl').html()),
                             editor: function(container, options){
                             var model = options.model;
                             if(model.status!=0){
                                 $('<span>'+model.endTimeSuccess+'</span>').appendTo(container);
                                return ;
                             }
                             var input = $('<input name=\'endTimeSuccess\' data-bind=\'value:endTimeSuccess\'>');
                             input.appendTo(container);
                             var widget = new uglcw.ui.ComboBox(input);
                             var dataSource=[];
                             dataSource.push({ text:'无效', value:0});
                             dataSource.push({ text:'有效', value:1});

                             widget.init({
                              dataTextFiled: 'text',
                              dataValueField: 'value',
                              change: function(){
                                    model.set('endTimeSuccessText', this.text());
                                    model.set('endTimeSuccess', this.value());
                              },
                              dataSource:dataSource
                             })
                             }">结束时间到,人数未满订单
                        </div>--%>
                    <div data-field="hasCount" uglcw-options="width: 80">已购数量</div>
...
<script>
    function save() {
        ...
        for (var i = 0; i < data.length; i++) {
            var item = data[i];
            if (!item.price) {
                uglcw.ui.error((i + 1) + "行,请输入拼团价");
                return false;
            }
            if (!item.count) {
                uglcw.ui.error((i + 1) + "行,请输入拼团人数");
                return false;
            }
        ...
        uglcw.ui.confirm('是否确定保存?', function () {
            uglcw.ui.loading()
            $.ajax({
                url: '${base}manager/shopTourWare/saveBatch?planId=' +${plan.id},
