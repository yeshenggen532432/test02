		...
		<div class="layui-col-md12">
			<div class="layui-card master">
				<div class="layui-card-header btn-group">
					...
					<div class="bill-info">
						<span class="no" style="color: green;">预付单据 ${billNo}</span>
				...
				<div class="layui-card-body">
					<form class="form-horizontal" uglcw-role="validator">
						...
						<div class="form-group">
							...
							<div class="col-xs-4">
								<input uglcw-role="textbox" id="totalAmt" readonly uglcw-model="totalAmt"
									   value="${totalAmt}"/>
							</div>
						</div>
						<div class="form-group" style="margin-bottom: 0px;">
							<label class="control-label col-xs-3">预付账户</label>
							<div class="col-xs-4">
				...
				<div class="layui-col-md5">
					<div class="layui-row layui-col-space5">
						<div class="layui-col-md4">
							...
							url:'${base}manager/queryIoTypeList?typeName=预付账款',
						...
						<div class="layui-col-md8">
							<div class="layui-card">
								<div class="layui-card-body full">
                                    ...
                                    responsive:['.master',40],
                                    url:'${base}manager/queryUseIoItemList?typeName=预付账款',
                                    criteria: '.query',
...
<script>
	...
	function add() {
		uglcw.ui.openTab('预付开单', '${base}manager/toFinPreOutEdit?r=' + new Date().getTime());
	}
	...
	function draftSave() {//暂存
		...
		if (!data.accId) {
			return uglcw.ui.warning('请选择预付账户');
		...
		$(items).each(function(idx, item){
			if(!item.amt){
				uglcw.ui.warning('第['+(idx+1)+']行,请输入金额');
				valid =false;
				return false;
			}
		})
			...
			$.ajax({
				...
				success: function (response) {
					...
					if (response.state) {
						...
						<%--setTimeout(function () {--%>
							<%--uglcw.ui.replaceCurrentTab('往来预付信息' + response.id, '${base}/manager/showFinPreOutEdit?billId=' + response.id);--%>
						<%--}, 1000);--%>
	...
	function audit() {
		...
		uglcw.ui.confirm('是否确定审核？', function () {
			uglcw.ui.loading();
			$.ajax({
				url: '${base}manager/updateFinPreOutAudit',
				type: 'post',
		...
		$.ajax({
			...
			success: function (json) {
				if(json.state){
					...
					{
						if(json.rows[i].itemName=='其他预付账款'){
							uglcw.ui.get('#grid').addRow({
