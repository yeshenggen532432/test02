                 ...
                 uglcw-options="
                    ...
                    id:'id',
                     ...
                     dblclick:function(row){
                       uglcw.ui.openTab('商品入仓信息'+row.id, '${base}manager/stkCrewIn/show?billId='+ row.id+$.map( function(v, k){  //只带id
                        return k+'='+(v||'');
                       }).join('&'));
                     },
                    url: '${base}manager/stkCrewIn/page',
...
<script type="text/x-uglcw-template" id="toolbar">
    <%--<c:if test="${permission:checkUserFieldPdm('stk.stkCrewIn.open')}">--%>
        <a role="button" href="javascript:add();" class="k-button k-button-icontext">
            <span class="k-icon k-i-track-changes-accept"></span>商品入仓单
        </a>
    <%--</c:if>--%>
    <%--<c:if test="${permission:checkUserFieldPdm('stk.stkCrewIn.show')}">--%>
...
<script>
    ...
    function add() {
        uglcw.ui.openTab('商品入仓单', '${base}manager/stkCrewIn/add');
    }

    function showDetail(id) {
        if (id) {
            uglcw.ui.openTab('商品入仓单' + id, '${base}manager/stkCrewIn/show?billId=' + id);
        } else {
