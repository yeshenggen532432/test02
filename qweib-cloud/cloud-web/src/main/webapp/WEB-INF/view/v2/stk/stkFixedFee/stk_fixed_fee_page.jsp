    ...
    <div class="layui-card header">
        <div class="lyaui-card-header actionbar btn-group">
            <a role="button" href="javascript:newBill();" class="primary k-button k-button-icontext">
                <span class="k-icon k-i-plus-outline"></span>固定费用开单
            </a>
        </div>
        <div class="layui-card-body full" style="padding-left: 5px;">
            <ul class="uglcw-query form-horizontal" id="export">
                <li>
                    ...
                    <input uglcw-role="textbox" uglcw-model="billNo" placeholder="费用单号">
                </li>
                <li>
                ...
                <li>
                    <select uglcw-role="combobox" uglcw-model="fixedId"
                            placeholder="固定费用类型"
                            uglcw-options="
                ...
                <li>
                    <input type="checkbox" uglcw-role="checkbox" id="showItems">
                    <label style="margin-top: 10px;" class="k-checkbox-label" for="showItems">显示明细</label>
                </li>
                <li>
            ...
            <div id="grid" uglcw-role="grid"
                 uglcw-options="{
                    ...
                    dblclick: function(row){
                        uglcw.ui.openTab('固定费用单'+row.id,'${base}manager/stkFixedFee/show?billId='+row.id);
                    },
                    url: '${base}manager/stkFixedFee/pages',
                    criteria: '.form-horizontal',
                    }">
                ...
                <div data-field="billDateStr" uglcw-options="width:160">费用归属日期</div>
                <div data-field="totalAmt" uglcw-options="width:100">费用金额</div>
                ...
                <div data-field="fixedCount" uglcw-options="width:250, hidden: true,
                         template: function(data){
                            return kendo.template($('#fixed-list').html())(data.list);
                         }
                        ">费用明细
                </div>
...
<script id="fixed-list" type="text/x-kendo-template">
    <table class="product-grid">
        <tbody>
        <tr style="font-weight: bold;">
            <td style="width: 120px;">固定费用名称</td>
            <td style="width: 60px;">金额</td>
        </tr>
        #for (var i=0; i< data.length; i++) { #
        <tr>
            <td>#= data[i].fixedName #</td>
            <td>#= data[i].amt #</td>
        </tr>
        # }#
        </tbody>
...
<script>
    $(function () {
        ...
        uglcw.ui.get('#showItems').on('change', function () {
            ...
            if (checked) {
                grid.showColumn('fixedCount');
            } else {
                grid.hideColumn('fixedCount');
            }
        });
    ...
    function cancelBill(id) {
        uglcw.ui.confirm('您确认要作废吗？', function () {
            $.ajax({
                url: "manager/stkFixedFee/cancel",
                type: "post",
                data: "billId=" + id,
        ...
        var rtn = uglcw.ui.Modal.open({
            ...
            yes: function (container) {
                ...
                if (validator.validate()) {
                    ...
                    url = '${base}manager/stkFixedFee/payFee';
    ...
    function newBill() {
        uglcw.ui.openTab('固定费用单', '${base}manager/stkFixedFee/add');
