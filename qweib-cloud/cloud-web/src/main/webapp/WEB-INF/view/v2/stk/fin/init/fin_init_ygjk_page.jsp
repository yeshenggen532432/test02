        ...
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid"
                 uglcw-options="
							...
							checkbox:true,
							pageable: true,
                    		url: '${base}manager/finInitQtWlMain/page',
                    		criteria: '.query',
                    		dataBound: function(){
                    	...
                    	">
                <div data-field="proName" uglcw-options="width:140,tooltip:true">往来对象</div>
                ...
                <div data-field="amt" uglcw-options="width:130">初始化借款金额</div>
                ...
                <div data-field="status" uglcw-options="width:80,template: uglcw.util.template($('#status').html())">状态
                </div>
...
<script>
    ...
    function auditBill(id, type) {
        ...
        if (type == 1) {
            title = '您确认要审核吗?';
            url = '${base}manager/finInitQtWlMain/auditBatch';
            data = "billId=" + id;
        } else if (type == 2) {
            title = '您确认要作废吗?';
            url = '${base}manager/finInitQtWlMain/cancelBill';
            data = "billId=" + id;
        }
        ...
        uglcw.ui.Modal.open({
            ...
            yes: function (container) {
                ...
                if (valid) {
                    ...
                    $.ajax({
                        url: '${base}/manager/finInitQtWlMain/item',
                        data: data,
                        type: 'post',
                        success: function (response) {
                            if (response.state) {
    ...
    function toAdd() {
        ...
        uglcw.ui.openTab(title, '${base}manager/finInitQtWlMain/add?bizType=${bizType}&proType=' + proType);
    }
    ...
    function download() {
        uglcw.ui.confirm('是否下载模版？', function () {
            window.location.href = '${base}manager/finInitQtWlMain/toWlJkInitAmtTemplate?bizType=${bizType}'
        })
    }
    ...
    function upload() {
        uglcw.ui.Modal.showUpload({
            url: '${base}manager/finInitQtWlMain/toUpWljkInitAmt?bizType=${bizType}',
            field: 'upFile',
            error: function (e) {
                uglcw.ui.notice({
                    type: 'error',
    ...
    //-------------------------------订阅：start---------------------------------------------
    //订阅
    uglcw.io.on('refreshInitYgjk-${bizType}', function (data) {
        uglcw.ui.get('#grid').reload();
    })
