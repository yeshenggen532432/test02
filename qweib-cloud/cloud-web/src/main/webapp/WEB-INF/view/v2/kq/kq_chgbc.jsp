                ...
                <div class="layui-card-body full">
                    <div id="grid" uglcw-role="grid"
                         ...
                         responsive:['.header',40],
                    		...
                    		url: '${base}manager/chgbc/queryKqChgBcPage',
                    		criteria: '.query',
                    	">
                        <div data-field="memberNm" uglcw-options="width:100">姓名</div>
                        <div data-field="banDateStr" uglcw-options="width:100">调班申请日期</div>
                        <div data-field="fromDate" uglcw-options="width:150">源日期</div>
                        <div data-field="bcName1" uglcw-options="width:150">源班次</div>
                        <div data-field="toDate" uglcw-options="width:100">目标日期</div>
                        <div data-field="bcName2" uglcw-options="width:100">目标班次</div>
                        ...
                        <%--<div data-field="ope" uglcw-options="width:100,template: uglcw.util.template($('#ope').html())">操作</div>--%>
                    </div>
                </div>
            </div>
        </div>
...
<script type="text/x-kendo-template" id="toolbar">
    <a role="button" href="javascript:add();" class="k-button k-button-icontext">
        <span class="k-icon k-i-add"></span>添加调班
    </a>
</script>
        ...
        <div class="layui-card-body">
            <form class="form-horizontal" uglcw-role="validator">
                ...
                <div class="form-group">
                    <label class="control-label col-xs-6">源日期</label>
                    <div class="col-xs-16">
                        <input uglcw-model="fromDate" uglcw-role="datepicker" id="fromDate"
                               onchange="onChangeDate(this.value,0);">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-6">源班次</label>
                    <div class="col-xs-16">
                        <input uglcw-model="bcName1" id="bcName1" uglcw-role="textbox" placeholder="" readonly>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-6">目标日期</label>
                    <div class="col-xs-16">
                        <input uglcw-model="toDate" uglcw-role="datepicker" id="toDate"
                               onchange="onChangeDate(this.value,1);">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-6">目标班次</label>
                    <div class="col-xs-16">
                        <input uglcw-model="bcName2" id="bcName2" uglcw-role="textbox" placeholder="" readonly>
                    </div>
                </div>
                <div class="form-group">
...
<script>
    ...
    function add() {
        ...
        uglcw.ui.Modal.open({
            ...
            yes: function (container) {
                ...
                var fromDate = uglcw.ui.get('#fromDate').value();
                var toDate = uglcw.ui.get('#toDate').value();
                var remarks = uglcw.ui.get('#remarks1').value();
                uglcw.ui.loading();
                $.ajax({
                    url: '${base}manager/chgbc/addKqChgBc',
                    type: 'post',
                    data: {"ids": memberId, "fromDate": fromDate, "toDate": toDate, "remarks": remarks},
                    async: false,
    ...
    function onChangeDate(dateStr, type) {
        ...
        $.ajax({
            url: "${base}manager/kqrpt/getKqBcByEmpIdAndDate",
            type: "POST",
            data: {"empId": memberId, "dateStr": dateStr},
            dataType: 'json',
            async: false,
            success: function (json) {
                if (json.state) {
                    if (type == 0) {
                        uglcw.ui.get('#bcName1').value(json.bcName);
                    } else if (type == 1) {
                        uglcw.ui.get('#bcName2').value(json.bcName);
                    }
                }
            }
        });
