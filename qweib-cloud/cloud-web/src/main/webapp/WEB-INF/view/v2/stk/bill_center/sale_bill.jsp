    ...
    <div class="layui-card">
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid" class="uglcw-grid-compact"
                ...
                <div data-field="outDate" uglcw-options="width:120">日期</div>
...
<script id="opt-tpl" type="text-x-uglcw-template">
    # if(data.outType){ #
    <button class="k-button k-success" style="width: 50px" onclick="printBill(#=data.id#)"><i class="k-icon"></i>打印(<span
            id="print_#= data.id#">0</span>)
    </button>
    # if(data.billStatus == '作废'){ #
    <button class="k-button k-info" style="width: 30px" onclick="copyBill(#=data.id#, '#=data.orderId#', '#=data.outType#')"><i
            class="k-icon"></i>复制
    </button>
    # } else if(data.billStatus == '暂存') {#
    <button class="k-button k-info" style="width: 30px" onclick="auditBill(this,#=data.id#, '#= data.billStatus#', '#= data.billNo#','#=data.stkId#')"><i
            class="k-icon"></i>审批
    </button>
    # } #
    # } #
...
<script>
    ...
    function printBill(id) {
        uglcw.ui.openTab('发票信息' + id, '${base}manager/showstkoutprint?fromFlag=1&billId=' + id)
    }
    ...
    function downloadDetails() {
        ...
        if (selection && selection.length > 0) {
            ...
            uglcw.ui.openTab('销售明细下载', '${base}manager/outWareListForItems?billNo=' + nos);
        } else {
                ...
                if (json.state) {
                    if (json.count > 0) {
                        ...
                        uglcw.ui.notice({
                            type: 'info',
                            title: '提示',
                            desc: "有业务员提交的订单未生成销售发票，<a style='color: #38F;' href='javascript:showSpecialOrder()'>立即查看>><a>"
                        });
                    }
                }
