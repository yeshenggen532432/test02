    ...
    <div class="layui-card">
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid"
                ...
                <div data-field="qrCode" uglcw-options="width:80,hidden:true,template: uglcw.util.template($('#qrCode-tpl').html())">
                    生成二维码
                </div>
...
<script type="text/x-uglcw-template" id="opt-tpl">
    <span uglcw-role="tag" onclick="showQrCode(#=data.id#,'#=data.name#')">生成二维码</span>
    #if (data.isDel==1){#
...
<script>
    ...
    function del(id, va) {
        ...
        uglcw.ui.confirm("是否确定" + text, function () {
            changeBean(va, 'isDel', id, function () {
                uglcw.ui.get('#grid').reload();
            });
        })
    }
    ...
    function showQrCode(id, name) {
        //二维码生成规则,分享后跳转页面
        var url = "${doMain}/web/mainWeb/toDiningUrl?diningid=" + id + "&gsId=${companyId}";
        uglcw.ui.Modal.open({
        ...
        <div class="layui-card-body">
            <form class="form-horizontal">
                <div class="form-group">
                    <label class="control-label col-xs-6">自动生成桌号</label>
                    <div class="col-xs-16">
...
<script type="text/x-uglcw-template" id="qrCode_tpl">
    <div class="layui-card">
        <div class="layui-card-body">
            <button class="downloadBtn" type="button" onclick="downloadImg('#=name#')">下载图片</button>
            <div id="qrcode"/>
        </div>
    </div>
    </div>
