        ...
        <div class="layui-card-body">
            <ul class="uglcw-query form-horizontal query">
                <li>
                    ...
                    <input type="hidden" uglcw-role="textbox" uglcw-model="dataType" value="${query.dataType}" id="dataType">
                </li>
            </ul>
        </div>
    </div>
        ...
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid"
                 uglcw-options="
                    ...
                    mergeBy:'billNo',
                    autoBind: true,
                    url: '${base}manager/customerSaleRecStat/statItemPages',
                    ...
                    ">
                <div data-field="billNo" uglcw-options="merge:true,width:150, tooltip: true, footerTemplate:'合计'">
                    <c:choose>
                    <c:when test="${query.dataType eq 1}">
                        发货单号
                    </c:when>
                    <c:otherwise>
                        销售单号
                ...
                <div data-field="proName" uglcw-options="merge:true,width:150,">客户名称</div>
                <div data-field="recAmt" uglcw-options="merge:true, format: '{0:n2}',attributes:{ class: 'router-link', 'data-field': 'recAmt'},width:120,footerTemplate: '#= uglcw.util.toString(data.recAmt || 0, \'n2\')#'">当期回款金额</div>
                <div data-field="totalRecAmt" uglcw-options="merge:true, format: '{0:n2}',width:120,footerTemplate: '#= uglcw.util.toString(data.totalRecAmt || 0, \'n2\')#'">累计回款金额</div>
                <div data-field="unRecAmt" uglcw-options="merge:true, width:120,format: '{0:n2}',footerTemplate: '#= uglcw.util.toString(data.unRecAmt || 0, \'n2\')#'">未回款金额</div>
                ...
                <div data-field="vehNo" uglcw-options="merge:true,width:100">配送车辆</div>
                <div data-field="stkName" uglcw-options="merge:true,width:100">出库仓</div>
                <div data-field="sourceBillNo" uglcw-options="merge:true,width:140,
                         template: function(dataItem){
                           return kendo.template($('#source-bill-no').html())(dataItem);
                          },
                ">对应销售单</div>
                <div data-field="remarks" uglcw-options="merge:true,width:100">备注</div>
                <div data-field="">
                </div>
            </div>
        </div>
...
<script>
    ...
    function onCellClick(field, row) {
        ...
        var billNo = row.sourceBillNo;
        if (field == "recAmt") {
            uglcw.ui.openTab('收款记录' + row.sourceBillId, '${base}manager/queryRecPageByBillId?dataTp=1&sdate='+sdate+'&edate='+edate+'&sourceBillNo=' + billNo);
        }
    }
