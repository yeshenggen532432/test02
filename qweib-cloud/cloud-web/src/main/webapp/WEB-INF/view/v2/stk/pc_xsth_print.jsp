...
<input type = "hidden" id="billId" value="${billId}"/>
  ...
  <input id="btnToPrint" value="打印" style=" width:70px; height:22px; font-size:16px;" type="button" onclick = "printit()">
</div>
      	...
      	<c:forEach items="${datas }" var="data">
      		<c:choose>
				...
				<c:when test="${data.fdFieldKey eq 'productDate'}">
					<td style="border:1px solid #000000;">
						<script>
							var productDate = ${item[data.fdFieldKey] };
							if(productDate){
								document.write(uglcw.util.toString(new Date(productDate), 'yyyy-MM-dd HH:mm:ss'))
							}
						</script>
					</td>
				</c:when>
...
<script type="text/javascript">
	...
	function onFieldChange(obj, id, name) {
		...
		var data = {
			id: id,
			fdModel: 'com.cnlife.stk.model.StkXsThsub',
		}
		data[name] = obj.value;
    ...
    function printit()
    {
		$.ajax({
			...
			data: {fdSourceId: '${billId}', fdSourceNo: '${billNo}', fdModel: 'com.cnlife.stk.model.StkXsThsub'},
			type: "post",
			success: function (json) {
				if ('${param.fromFlag}' == 0) {
	...
	function changeFdWidth(obj,id){
		...
		$.ajax({
			...
			data: {fdSourceId: '${billId}', fdModel: 'com.cnlife.stk.model.StkXsThsub'},
			type: "post",
			async: true,
			success: function (json) {
				document.getElementById("btnToPrint").value = "打印(" + json.count + ")";
			}
		});
</script>
</body>
