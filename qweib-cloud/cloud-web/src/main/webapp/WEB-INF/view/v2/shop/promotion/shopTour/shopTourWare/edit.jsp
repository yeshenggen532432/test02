    ...
    <div id="app">
        <div style="padding-bottom: 50px;">
            <i-form ref="form" :label-width="120" :model="shopTourWare" :rules="rules">
                ...
                <form-item class="main-item" label="拼团名称" prop="name">
                    <i-select v-model="shopTourWare.planId"  placeholder="选择拼团方案" @on-change="selectTourPlan" style="width:120px">
                        <i-option v-for="item in tourPlans" :value="item.id" :key="item.name">{{ item.name }}</i-option>
                    </i-select>
                    <i-input :disabled="shopTourWare.status == 1" style="width: 200px;" required v-model="shopTourWare.name"
                ...
                <form-item class="main-item" label="生效时间">
                    <div style="display: flex;">
                        <form-item prop="startTime">
                            <date-picker :disabled="shopTourWare.status == 1" :options="startTimeOptions" type="datetime"
                ...
                <form-item  class="main-item"  label="方案结束,人数未满订单" prop="count">
                    <input type="radio" v-model="shopTourWare.endTimeSuccess" value="0"/>无效<span style="padding-left: 10px;color: red;">已支付订单自动取消并自动退款用户</span><br/>
                    <input type="radio" v-model="shopTourWare.endTimeSuccess"  value="1"/>有效<span style="padding-left: 10px;color: red;">已支付订单自动买单生效</span>
                   <%-- <span style="padding-left: 10px;color: red;">
                        <span v-if="shopTourWare.endTimeSuccess==0">选择“无效”时，拼团时间结束后，拼团订单自动取消并自动退款用户。</span>
                        <span v-if="shopTourWare.endTimeSuccess==1">已买的订单自动生效买单。</span>

                    </span>--%>
                </form-item>
                <form-item  class="main-item"  label="参团人数" prop="count">
                    <input-number :disabled="shopTourWare.status == 1" :min="2" :max="100" :active-change="false"
                ...
                <form-item  class="main-item"  label="拼团价格" prop="price">
            ...
            vm = new Vue({
                ...
                data: {
                    ...
                    rules: {
                        ...
                        startTime: [
                            {required: true, message: '请选择拼团开始时间'}
                        ],
                        endTime: [
                            {required: true, message: '请选择拼团结束时间'}
                        ]
                    },
                    shopTourWare: {
                ...
                methods: {
                    ...
                    load: function () {
                        ...
                        vm.$Spin.show();
                        $.ajax({
                            url: '${base}manager/shopTourWare/get?id=' + vm.shopTourWare.id,
                            type: 'get',
                    ...
                    loadTourPlans: function () {
                        var vm = this;
                        $.ajax({
                            url: '${base}manager/shopTourPlan/list',
                            type: 'get',
                            dataType: 'json',
                    ...
                    save: function () {
                        ...
                        vm.$refs.form.validate(function (v) {
                            if (v) {
                                ...
                                $.ajax({
                                    url: '${base}manager/shopTourWare/save',
                                    type: 'post',
                                    ...
                                    success: function (response) {
                                        if (response.code === 200) {
                                            ...
                                            uglcw.io.emit('shop_tour_ware_chage',1);
                                        } else {
                                            vm.$Message.error(response.message);
                                        }
                                        vm.$Spin.hide();
