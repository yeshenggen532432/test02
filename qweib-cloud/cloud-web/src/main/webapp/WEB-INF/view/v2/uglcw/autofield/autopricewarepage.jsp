...
<html>
	...
	<body>
		...
		<table id="datagrid" class="easyui-datagrid" fit="true" singleSelect="true"
			url="manager/queryCompanyStockWares?wtype=${wtype}" border="false"
		...
		<script type="text/javascript">
		 ...
		 function initGrid()
		    ...
		    function formatAutoAmt(val,row,index){
		    	...
		    	var html ="<input type='hidden' id='auto_price_Id_"+row.wareId+"_"+this.value+"'/><input type='text' name='autoPrice' size='7' onchange='changeWareAutoPrice("+row.wareId+","+this.value+")' id='auto_Price_"+row.wareId+"_"+this.value+"' value='"+price+"' />";
		    	if(autoPriceDatas.length>0){
		    		for(var i=0;i<autoPriceDatas.length;i++){
		    			...
		    			if(json.wareId==row.wareId&&json.autoId==this.value){
		    				html ="<input type='hidden' id='auto_price_Id_"+row.wareId+"_"+this.value+"' value='"+json.id+"'/><input type='text' name='autoPrice' size='7' onchange='changeWareAutoPrice("+row.wareId+","+this.value+")' id='auto_Price_"+row.wareId+"_"+this.value+"' value='"+json.price+"' />";
		    				break;
		    			}
		    		}
		    	}
