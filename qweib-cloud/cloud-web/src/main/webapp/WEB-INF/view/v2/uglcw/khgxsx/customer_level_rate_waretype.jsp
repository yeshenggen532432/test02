	...
	<div class="uglcw-layout-container">
		<div class="uglcw-layout-fixed" style="width:200px">
			<div class="layui-card">
				<div class="layui-card-header">设置商品类别价格折扣率<span style="color: red">注：红色表示下级类别有设置折扣率</span></div>
...
<script id="type_rate_template" type="text/x-uglcw-template">
	...
	#if(item.id!=0){#
	&nbsp;<input id="type_${relaId}_#= item.id#" uglcw-role="numeric" style="width:80px;" uglcw-options="min:0, max: 1000"  onchange="changeRate(this,'#= item.id#')"  />%
	#}#
</script>
...
<script>
	...
	function changeRate(o,itemId) {
		$.ajax({
			url: "manager/updateCustomerLevelWareTypeRate",
			type: "post",
	...
	function laodData()
	{
		var path = "${base}/manager/customerLevelRateList";
		var relaId = '${relaId}';
		if(relaId==""){
			return;
		}
					...
					if(list){
						$.map(list, function (item) {
							if(item.rate!=''&&item.rate!=0){
								//console.log("#type_"+item.relaId+"_"+item.waretypeId);
								uglcw.ui.get("#type_"+item.relaId+"_"+item.waretypeId).value(item.rate);
								//$("#type_"+item.relaId+"_"+item.waretypeId).value(item.rate);
								//document.getElementById("type_"+item.relaId+"_"+item.waretypeId+"").value=item.rate;
							}
						})
					}
				}
	...
	{
		var path = "${base}/manager/queryLevelSubTypeRateList";
		var relaId = '${relaId}';
		if(relaId==""){
			return;
		}
