...
<script type="text/x-kendo-template" id="toolbar">
    <a role="button" href="javascript:initclick();" class="k-button k-button-icontext">
        <span class="k-icon k-i-dollar"></span>充值
    </a>
</script>
        ...
        <div class="layui-card-body">
            <form class="form-horizontal" uglcw-role="validator">
                <div class="form-group">
                    ...
                    <div class="col-xs-12">
                        <input uglcw-model="inputCash" id="inputCash" uglcw-role="textbox">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-8">赠送金额:</label>
                    <div class="col-xs-12">
                        <input uglcw-model="freeCost" uglcw-role="textbox" id="freeCost">
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-8">新卡类型</label>
                    <div class="col-xs-12">
                        <tag:select2 id="cardTypeSel" name="cardType" displayKey="id"
                                     headerValue="请选择卡类型" headerKey="0" displayValue="type_name"
                                     tableName="shop_member_type"/>
                    </div>
                </div>
                <div class="form-group">
                    <label class="control-label col-xs-8">支付类型</label>
                    <div class="col-xs-12">
                        <select uglcw-role="combobox" uglcw-model="payType" id="payType">
    ...
    function initclick() {
        ...
        uglcw.ui.Modal.open({
            ...
            yes: function (container) {
                ...
                if (data.inputCash == 0 && data.freeCost == 0) {
                ...
                data.wxPay = 0;
                data.zfbPay = 0;
                if (paytype == "微信支付") data.wxPay = data.inputCash;
                if (paytype == "现金支付") data.cashPay = data.inputCash;
                if (paytype == "银行卡") data.bankPay = data.inputCash;
                if (paytype == "支付宝") data.zfbPay = data.inputCash;
                uglcw.ui.loading();
                $.ajax({
                    url: '${base}manager/pos/posMemberInput',
                    type: 'post',
                    data: data,
