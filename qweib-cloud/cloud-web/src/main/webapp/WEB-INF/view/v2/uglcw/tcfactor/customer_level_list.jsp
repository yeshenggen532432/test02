...
<script id="oper1" type="text/x-uglcw-template">
    ...
    <select uglcw-role="combobox" style="width: 80%;" class="factor-combo" id="factor-#= data.type##= data.id#"
         ...
         template: function(item){
            var id = #= data.id#, name = '#= data.khdjNm#';
            item.id = id;
            item.name = name;
            item.type = '#= data.type#';
...
<script>
    ...
    function setWareTypeFactor(id, name,type) {
        uglcw.ui.openTab(name + "_设置商品类别提成系数", "${base}manager/customerLevelTcRatewaretype?_sticky=v2&type="+type+"&relaId=" + id);
    }

    function setWareFactor(id, name,type) {
        uglcw.ui.openTab(name + "_设置商品提成系数", "${base}manager/toCustomerLevelTcFactorSetWareTree?_sticky=v2&type="+type+"&relaId=" + id);
    }
