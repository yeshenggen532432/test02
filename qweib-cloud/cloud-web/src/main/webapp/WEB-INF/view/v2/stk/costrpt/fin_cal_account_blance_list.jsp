	...
	<c:if test="${not empty datas}">
			<c:forEach items="${datas}" var="data">
				<c:if test='${!(data["qc_amt"] eq 0 && data["bq_in_amt"] eq 0 && data["bq_out_amt"] eq 0 && data["qm_amt"] eq 0)}'>
					<tr ondblclick="showDetail('${data["name"]}','${data["id"]}')">
						<td style="padding-left: 20px;">&nbsp;&nbsp;${data["name"]}</td>
						<td >
							<fmt:formatNumber value='${data["qc_amt"]}' pattern="#,#00.0#"/>
						</td>
						<td >
							<fmt:formatNumber value='${data["bq_in_amt"]}' pattern="#,#00.0#"/>
						</td>
						<td >
							<fmt:formatNumber value='${data["bq_out_amt"]}' pattern="#,#00.0#"/>
						</td>
						<td>
							<fmt:formatNumber value='${data["qm_amt"]}' pattern="#,#00.0#"/>
						</td>
						</td>
					</tr>
				</c:if>
...
<script>
    ...
    function showDetail(tabName,id){
        var sdate = $("#sdate").val();
        var edate = $("#edate").val();
        var rptType = $("#rptType").val();
        var typeId = $("#typeId").val();
        var url = "<%=basePath %>/manager/calAccountBlanceListItems?sticky=v2&typeName="+tabName+"&sdate="+sdate+"&edate="+edate+"&rptType="+rptType+"&typeId="+typeId;
        if(rptType==11){
            url = "<%=basePath %>/manager/toFinAccIo?sticky=v2&sdate="+sdate+"&edate="+edate+"&accId="+id;
        }
