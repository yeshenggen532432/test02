...
<div class="layui-fluid page-list">
    <div class="layui-card header">
        <div class="layui-card-header actionbar btn-group uglcw-query">

            <c:set var="showOfficeFix" value="${permission:checkUserButtonPdm('stk.stkOut.officeFix')}"/>
            <c:set var="showFinanceFix" value="${permission:checkUserButtonPdm('stk.stkOut.financeFix')}"/>

<%--            <c:set var="showOfficeFix" value="true"/>--%>
<%--            <c:set var="showFinanceFix" value="true"/>--%>
            ...
            <c:if test="${permission:checkUserButtonPdm('stk.stkOut.createxs')}">
                <a role="button" href="javascript:addRed(0);" style="color: white" class="primary-red k-button k-button-icontext">
                    <span class="k-icon k-i-file-add"></span>红字开单
                </a>
            </c:if>
            <c:if test="${showOfficeFix}">
                <a role="button" class="k-button k-button-icontext" href="javascript:fixBill(1);">
                    <span class="k-icon k-i-audio"></span>文员确认
                </a>
            </c:if>
            <c:if test="${showFinanceFix}">
            <a role="button" class="k-button k-button-icontext" href="javascript:fixBill(2);">
                <span class="k-icon k-i-audio"></span>财务确认
            </a>
            </c:if>
            ...
            <a role="button" class="k-button k-button-icontext" href="javascript:saleOutWareStat();">
                <span class="k-icon k-i-print"></span>销售汇总统计
            </a>
            <a role="button" class="k-button k-button-icontext"
               href="javascript:downloadDetails();">
                <span class="k-icon k-i-download"></span>下载销售单据明细
            </a>
            <a id="orderWaitCountSpan" role="button" class="k-button k-button-icontext"
               href="javascript:showSpecialOrder();">
                <i uglcw-role="tooltip"
                   style="color: red; cursor: pointer;margin-bottom: 3px;" id="customer-dept-tip"
                   class="k-icon k-i-info"></i>
                查询未生成销售的订单
            </a>
<%--            href="javascript:showColumnSetting('static/uglcu/biz/erp/columnView/stk_out_column_view.json','XSFP_SHOW_COLUMN');"--%>
            <a role="button" id="showColumnSetting" class="k-button k-button-icontext" >
                <span class="k-icon k-i-settings"></span>设置显示列
            </a>

            <select style="width: 160px;height: 29px;" class="biz-type" uglcw-model="saleType" uglcw-role="combobox"
                    uglcw-options="placeholder:'业务类型', value: '', change: function(){
            ...
            <a href="javascript:maxMin(this);" class="k-button">
                <span class="k-icon uglcw-grid-maxmin ion-md-expand"></span>
            </a>
        </div>
            ...
            <ul class="uglcw-query">
                ...
                <li>
                    <tag:select2 name="stkId" id="stkId" tableName="stk_storage" headerKey="-1"
                                 headerValue=""
                                 index="0" whereBlock="status is null or status=1"
                ...
                <li>
                    <select id="saleCar" uglcw-options="value:'0', tooltip: '仓库类型'" uglcw-model="saleCar" uglcw-role="combobox"
                            placeholder="仓库类型">
                ...
                <li style="display: ${(showOfficeFix and showFinanceFix)?'':'none'}">
                    <select uglcw-role="combobox" uglcw-model="fixStep"
                            uglcw-options="value: ''"
                            placeholder="确认阶段">
                        <option value="0" >初始阶段</option>
                        <option value="1">文员确认</option>
    ...
    <div class="layui-card">
        <div class="layui-card-body full">
            <div id="grid" uglcw-role="grid" class="uglcw-grid-compact"
                ...
                <div data-field="outDate" uglcw-options="width:120">单据日期</div>
                <div data-field="khNm" uglcw-options="width:180, tooltip: true">客户名称</div>
                <div data-field="billStatus" uglcw-options="width:80">单据状态</div>
                <div data-field="operator" uglcw-options="width:70">创建人</div>
                <div data-field="staff" uglcw-options="width:70">业务员</div>
                <div data-field="newTime"
                     uglcw-options="width:90, template: uglcw.util.template($('#modified').html())">是否修改过
                </div>
                <div data-field="fixStep"
                     uglcw-options="width:90,
                      hidden:true,
                      template: uglcw.util.template($('#fixStep').html())">确认阶段
                </div>
                <div data-field="op" uglcw-options="width:110,
                ...
                <c:if test="${permission:checkUserFieldPdm('stk.stkOut.lookamt')}">
                    ...
                    <div data-field="disAmt" uglcw-options="width:80, footerTemplate: '#= data.disAmt#'">销售金额</div>
                ...
                <div data-field="saleType" uglcw-options="width:100,template:uglcw.util.template($('#saleTypeTpl').html())">
                    业务类型
                </div>
                <div data-field="orderNo" uglcw-options="width:135">订单单号</div>
                <div data-field="outType" uglcw-options="width:80">出库类型</div>
...
<script id="fixStep" type="text/x-uglcw-template">
    # if(data.fixStep==0){ #
    <span>初始阶段</span>
    # }else if(data.fixStep==1){ #
    <span>文员确认</span>
    #}else if(data.fixStep==2){#
    <span>财务确认</span>
    #}#
</script>

<script id="saleTypeTpl" type="text/x-uglcw-template">
    # if(data.saleType=='001'){ #
    <span>传统业务类</span>
    # }else if(data.saleType=='003'){ #
    <span>线上商城</span>
    # }else if(data.saleType=='004'){ #
    <span>线下门店</span>
    #}#
</script>
...
<script id="opt-tpl" type="text-x-uglcw-template">
    # if(data.outType){ #
    <button class="k-button k-success" style="width: 50px" onclick="printBill(#=data.id#)"><i
            class="k-icon"></i>打印(<span
            id="print_#= data.id#">0</span>)
    </button>
    # if(data.billStatus == '作废'){ #
    <button class="k-button k-info" style="width: 30px"
            onclick="copyBill(#=data.id#, '#=data.orderId#', '#=data.outType#')"><i
            class="k-icon"></i>复制
    </button>
...
<script id="bill-no" type="text/x-kendo-template">
    ...
    <div class="bill-tags">
        #if(data.remarks){#
        <span class="bill-tag bill-tag-remark">
...
<script>
    ...
    $(function () {
        ...
        $('#grid').kendoTooltip({
            filter: '.bill-tag-remark',
            position: 'right',
            content: function (e) {
                var row = uglcw.ui.get('#grid').k().dataItem($(e.target).closest('tr'));
                return '<span style="color: #ff984f;font-size: 14px;font-weight: bold; margin-right: 5px">备注:</span>' + row.remarks;
            }
        });

        if(${showOfficeFix} || ${showFinanceFix}){
            grid.showColumn('fixStep');
        }
        /*显示列相关设置*/
        initColumnShow('XSFP_SHOW_COLUMN');//过滤设置隐藏的列表
        $("#showColumnSetting").click(function(){
            //设置显示列
            showColumnSetting('static/uglcu/biz/erp/columnView/stk_out_column_view.json','XSFP_SHOW_COLUMN');
        });
        /*显示列相关设置*/
    ...
    function maxMin() {
        var $master = $('.header .layui-card-body');
        var grid = uglcw.ui.get('#grid');
    ...
    function addRed(orderId) {
        uglcw.ui.openTab('销售红单', "${base}manager/pcstkout?redMark=1&&orderId=0");
    }
            ...
            $.ajax({
                ...
                success: function (response) {
                    if (!response.state) {
                        ...
                        uglcw.ui.warning('该单据是由销售订单生成的销售单，不能复制，只有作废该单，才能复制!')
                    }
                }
            })
        }
    ...
    function batchPrint() {
        var selection = uglcw.ui.get('#grid').selectedRow();
        if (selection && selection.length > 0) {
            uglcw.ui.openTab('批量打印单据', '${base}manager/showstkoutbatchprint?fromFlag=1&billIds=' + $.map(selection, function (row) {
                return row.id
            }).join(','));
        } else {
            uglcw.ui.info('请勾选单据');
    ...
    function saleOutWareStat() {
        var selection = uglcw.ui.get('#grid').selectedRow();
        if (selection && selection.length > 0) {
            uglcw.ui.openTab('销售汇总统计', '${base}manager/showstkoutbatchprint?fromFlag=1&showGroupWare=1&billIds=' + $.map(selection, function (row) {
                return row.id
            }).join(','));
        } else {
            uglcw.ui.info('请勾选单据');
        ...
        uglcw.ui.confirm('是否确定审核[' + no + ']？', function () {
            checkProductStock(stkId, wareList, function () {
                $.ajax({
                    ...
                    type: 'post',
                    data: {billId: id, changeOrderPrice: 1, autoCreateFhd: 1},
                    dataType: 'json',
                    success: function (response) {
                        if (response.state) {
    ...
    function fixBill(fixStep){
        ...
        if (selection && selection.length > 0) {
            ...
            billId = row.id;
            billNo = row.billNo;
            if(row.fixStep==1&&fixStep==1){
                uglcw.ui.info("文员已确认，不能操作");
                return;
            }
            if(row.fixStep==2){
                uglcw.ui.info("财务已确认，不能操作");
                return;
            }
        }


        uglcw.ui.confirm('是否确定操作'+billNo, function () {
            $.ajax({
                url: '${base}manager/fixBill',
                type: 'POST',
                data: {
                    fixStep: fixStep,
                    billId:billId
    ...
    function printBill(id) {
        uglcw.ui.openTab('单据信息' + id, '${base}manager/showstkoutprint?fromFlag=1&billId=' + id)
    }

    function downloadDetails() {
        var selection = uglcw.ui.get('#grid').selectedRow();
        if (selection && selection.length > 0) {
            var nos = $.map(selection, function (row) {
                return "'" + row.billNo + "'";
            }).join(',');
            uglcw.ui.openTab('单据明细下载', '${base}manager/outWareListForItems?billNo=' + nos);
        } else {
            uglcw.ui.warning('请勾选要下载的单据');
        }
    }
    ...
    function getWaitToSaleBillOrderCount() {
        $.ajax({
            ...
            success: function (json) {
                if (json.state) {
                    if (json.count > 0) {
                        //uglcw.ui.warning("有业务员提交的订单未生成销售发票，请去查看<a href='javascript:showSpecialOrder()'>>><a>");
                        uglcw.ui.notice({
                            type: 'info',
                            title: '提示',
                            desc: "有业务员提交的订单未生成销售单据，<a style='color: #38F;' href='javascript:showSpecialOrder()'>立即查看>><a>"
                        });
                    }
                }
