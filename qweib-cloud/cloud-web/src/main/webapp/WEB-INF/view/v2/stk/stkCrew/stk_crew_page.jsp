                 ...
                 uglcw-options="
                    ...
                    id:'id',
                     ...
                     dblclick:function(row){
                       uglcw.ui.openTab('库位理货信息'+row.id, '${base}manager/stkCrew/show?billId='+ row.id+$.map( function(v, k){  //只带id
                        return k+'='+(v||'');
                       }).join('&'));
                     },
                    url: '${base}manager/stkCrew/page',
...
<script type="text/x-uglcw-template" id="toolbar">
    <%--<c:if test="${permission:checkUserFieldPdm('stk.stkCrew.open')}">--%>
        <a role="button" href="javascript:add();" class="k-button k-button-icontext">
            <span class="k-icon k-i-track-changes-accept"></span>库位理货单
        </a>
    <%--</c:if>--%>
    <%--<c:if test="${permission:checkUserFieldPdm('stk.stkCrew.show')}">--%>
...
<script>
    ...
    function add() {
        uglcw.ui.openTab('库位理货单', '${base}manager/stkCrew/add');
    }

    function showDetail(id) {
        if (id) {
            uglcw.ui.openTab('库位理货单' + id, '${base}manager/stkCrew/show?billId=' + id);
        } else {
