        ...
        <div class="layui-col-md12">
                ...
                <div class="layui-card header">
                    <div class="layui-card-body">
                        <ul class="uglcw-query form-horizontal query">
                            <li>
                                <input uglcw-model="name" uglcw-role="textbox" placeholder="品牌名称">
                            </li>
                            <li>
            ...
            <div class="layui-card">
                <div class="layui-card-body full">
                    ...
                    id:'id',
                    url: '${base}manager/toBrandPage',
                    criteria: '.query',
                    pageable: true,

                    }">

                        <div data-field="name">品牌名称</div>
                        <div data-field="sp_oper"
                             uglcw-options="template: uglcw.util.template($('#sp_oper').html())">品牌相关操作
                        </div>
                    </div>
                </div>
            </div>
        ...
        <div class="layui-card-body">
            <div class="form-horizontal" uglcw-role="validator" id="brand_name">
                <div class="form-group">
                    ...
                    <label class="control-label col-xs-8">品牌名称</label>
                    <div class="col-xs-14">
                        <input class="form-control" uglcw-role="textbox" uglcw-model="name" uglcw-validate="required">
                    </div>
                </div>
            </div>
        </div>
...
<script>
    ...
    function toDel() {   //删除
        ...
        if (selection) { //判断是否为空
            uglcw.ui.confirm('是否要删除选中的品牌名称?', function () {
                $.ajax({
                    url: '${base}/manager/deleteBrandById',
                    data: {
                        id: selection[0].id
                    },
                    type: 'post',
    ...
    function addWare(id) {
        uglcw.ui.Modal.showGridSelector({
            closable: false,
            title: false,
            url: '${base}manager/dialogShopWaresPage?brandId=' + id+'&type=1',
            ...
            yes: function (nodes) {
                if(nodes && nodes.length>0){
                    ...
                    callBackFunSelectWare(ids, id,1);
                };
    ...
    function delWare(id) {
        uglcw.ui.Modal.showGridSelector({
            closable: false,
            title: false,
            url: '${base}manager/dialogShopWaresPage?brandId=' + id+'&type=2',
            query: function (params) {
                params.status = 1;
            },
            checkbox: true,
    ...
    //回调
    function callBackFunSelectWare(ids, id, type) {
        $.ajax({
            url: "${base}manager/batchUpdateShopWareBrand",
            data: { ids:ids.join(','),
                brandId:id},
            type: "post",
            success: function (json) {
