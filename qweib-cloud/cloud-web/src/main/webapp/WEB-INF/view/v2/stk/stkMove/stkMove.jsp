            ...
            <ul uglcw-role="buttongroup">
                ...
                <c:if test="${permission:checkUserFieldPdm('stk.stkMove.audit')}">
                    <li onclick="audit()" id="btnaudit" class="k-info" data-icon="track-changes-accept"
                        style="display: ${(stkMove.id ne 0 and stkMove.status eq -2)?'':'none'}">审批
                    </li>
                </c:if>
                <c:if test="${permission:checkUserFieldPdm('stk.stkMove.cancel')}">
                    <li onclick="cancel()" id="btncancel" class="k-info" data-icon="delete"
                        style="display: ${stkMove.id eq 0 or stkMove.status eq 2?'none':''}">作废
                    </li>
                </c:if>

                    <c:if test="${permission:checkUserFieldPdm('stk.stkMove.print')}">
                        <li onclick="toPrint()" id="btnprint" class="k-info" data-icon="print"
                            style="display: ${stkMove.id eq 0?'none':''}">打印
                        </li>
                    </c:if>

            </ul>
            <div class="bill-info">
                <div class="no" style="color:green;">
                    <span id="billNo" uglcw-model="billNo" style="height: 25px;" uglcw-role="textbox">${stkMove.billNo}</span>
                </div>
                <div class="status" style="color:red;">
                    <c:choose>
                        <c:when test="${stkMove.status eq -2 and not empty stkMove.id and stkMove.id ne 0}"><span
                                id="billStatus" style="height: 25px;width: 80px"  uglcw-model="billstatus"
                                uglcw-role="textbox">暂存</span></c:when>
                        <c:when test="${stkMove.status eq 1}"><span id="billStatus" style="height: 25px;width: 80px"
                                                                      uglcw-model="billstatus" uglcw-role="textbox"
                                                                    >已审批</span></c:when>
                        <c:when test="${stkMove.status eq 2}"><span id="billStatus" style="height: 25px;width: 80px"
                                                                      uglcw-model="billstatus" uglcw-role="textbox">已作废</span></c:when>
                        <c:otherwise>
        ...
        <div class="layui-card-body">
            <form class="form-horizontal" uglcw-role="validator">
                ...
                <div class="form-group">
                    <label class="control-label col-xs-5 col-md-3">移库时间</label>
                ...
                <div class="form-group" style="display: none;">
                    ...
                    <label class="control-label col-md-3">移库金额</label>
                    <div class="col-md-4">
...
<script>
    $(function () {
        ...
        uglcw.ui.get('#inDate').value('<fmt:formatDate value="${now}" pattern="yyyy-MM-dd HH:mm" />');
        </c:if>
    ...
    function add() {
        uglcw.ui.openTab('移库开单', '${base}manager/stkMove/add?billId=0&r=' + new Date().getTime());
    }
    ...
    function toPrint() {
        var billId = uglcw.ui.get("#billId").value();
        uglcw.ui.openTab('打印移库单${stkMove.id}', '${base}manager/stkMove/print?billId=' + billId);
    }
