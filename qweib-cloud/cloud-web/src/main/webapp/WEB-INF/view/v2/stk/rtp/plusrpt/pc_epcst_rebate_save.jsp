    ...
    <div class="layui-card-body full">
        <div id="grid" uglcw-role="grid"
             uglcw-options="
                    ...
                    ">
            <div data-field="khNm" uglcw-options="width:100, tooltip: true,footerTemplate: '合  计:'">二批客户名称</div>
            <div data-field="wareNm" uglcw-options="width:100,tooltip: true">商品名称</div>
            <div data-field="outQty"
                 uglcw-options="width:100,format: '{0:n2}',footerTemplate: '#= uglcw.util.toString(data.outQty,\'n2\')#'">
                发货数量
            </div>
            <div data-field="rate"
                 uglcw-options="width:100,format: '{0:n2}',footerTemplate: '#= uglcw.util.toString(data.rate,\'n2\')#'">单件返利
            </div>
            <div data-field="amt"
                 uglcw-options="width:100,format: '{0:n2}',footerTemplate: '#= uglcw.util.toString(data.amt,\'n2\')#'">返利总额
            </div>
        </div>
    </div>
</div>
...
<script>
    ...
    function saveRpt() {//保存
        ...
        var paramStr = dateTypeStr + ":" + data.sdate + "-" + data.edate + " 销售类型:" + data.outType + "客户名称:" + data.khNm
            + "客户类型:" + data.customerType + "商品名称:" + data.wareNm;
        var merCols = 'khNm';
        var saveHtml = {
            paramStr: paramStr,
            merCols: merCols,
